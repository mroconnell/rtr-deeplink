...
The production resolver version observed during this pass was `5da09b574fd6a5acdc485bdddb5991cb32b1f550`, before this shared-host change. The two accepted LMC queue candidates are preserved in `yesgov_694_pending_queue_2026-09-15.csv`, outside the active `scripts/tier3_auto_transcription_queue.txt`. Add them to the active queue only after the app change merges and a deployment with these exact pins is verified. The existing page-6590 manual override is already live and protected against re-ingest overwrite.
