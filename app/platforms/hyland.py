...
# Hyland "OnBase Agenda Online" -- confirmed 2026-08-16 across 3 real
# customers on 3 different hosting domains, all serving the identical
# vendor template: tucsonaz.hylandcloud.com (Tucson, AZ), mccobagenda.
# databankcloud.com (Maricopa County, AZ), agendanet.saccounty.gov
# (Sacramento County, CA). All confirmed via plain `curl` -- no headless
# browser needed for any of the 3, overturning BACKLOG.md's earlier
...
#
# Real structure, confirmed live against all 3:
...
#   agenda-item ids to a real video-seek offset in seconds (Maricopa/
#   Sacramento only, absent on Tucson since there's no video to seek).
...
#   `type` uses the same value as `doctype`, confirmed across all 3) is
#   plain server-rendered HTML with NO JS execution needed: a real
#   `<h1>{meeting name}<br>{date time}<br></h1>` header, plus a full nested
#   outline of `accessible-item`/`accessible-section` divs, each item
#   carrying its own real text and a `loadAgendaItem({id})` onclick whose
#   `{id}` is the same id used as a key in `itemEventPoints` above -- the
#   two pages join on that id to produce a real, timestamped agenda outline
#   with no headless browser involved.
# - No caption/transcript track of any kind was found on any of the 3
#   samples' JW Player config (no `tracks:` key at all, unlike TelVue) --
#   this platform is video(+agenda)-only until a real example says
#   otherwise.
# - Jurisdiction: none of the 3 samples has reliable in-page jurisdiction
#   text (Maricopa/Tucson have zero; Sacramento's <title> happens to carry
#   "Sacramento County" but that's a single, unconfirmed-to-generalize
#   example) -- same reasoning as this file's LIMS/lookup_by_domain()
#   precedent, so all 3 known domains are registered in
...
_TITLE_DATE_RE = re.compile(
    r"<h1[^>]*>\s*([^<]+?)\s*<br\s*/?>\s*"
    r"(\d{1,2}/\d{1,2}/\d{4}\s+\d{1,2}:\d{2}:\d{2}\s*[AP]M)\s*<br",
    re.IGNORECASE,
)
...
class HylandAssetFinder(AssetFinder):
    ...
    async def resolve(self, url: str) -> ResolvedMeeting:
        ...
        async with aiohttp.ClientSession(headers=self.headers) as session:
            ...
            agenda_html: Optional[str] = None
            agenda_url: Optional[str] = None
            if meeting_id and doctype and path_match:
                agenda_url = f"{path_match.group(1)}ViewMeetingAgenda?meetingId={meeting_id}&type={doctype}"
                agenda_url = f"{parsed.scheme}://{parsed.netloc}{agenda_url}"
                agenda_html = await self._fetch(session, agenda_url)

        title, date = self._extract_title_date(agenda_html or html)
        agenda_items = self._build_agenda_items(agenda_html, event_points)
        ...
        agenda_link = agenda_url if agenda_url and not agenda_items else None
    ...
    def _extract_title_date(html: Optional[str]):
        ...
        for fmt in ("%m/%d/%Y %I:%M:%S %p",):
            try:
                date = datetime.strptime(match.group(2).strip(), fmt).strftime("%Y-%m-%d")
            except ValueError:
                pass
        return title, date
    ...
    @staticmethod
    def _build_agenda_items(agenda_html: Optional[str], event_points: dict) -> List[TranscriptSegment]:
        if not agenda_html or not event_points:
            return []
        items = []
        for item_id, text in _AGENDA_ITEM_RE.findall(agenda_html):
            start = event_points.get(item_id)
            text = " ".join(html_module.unescape(text).split())
