...
class VimeoAssetFinder(AssetFinder):
    ...
    def _jurisdiction(oembed: Optional[dict]) -> Optional[str]:
        ...
        `jurisdiction_enrich.strip_institutional_suffix()`'s own comment)
        is stripped first, since the suffix is never part of the place's
        own proper name. Run through the same Census-validated
        `validated_label_extract()` every other adapter uses, which
