...
# --- WO-935: a finished OWN-transcription job is compared with its video -----
#
# Real numbers: Leon Valley TX show 179 (page 1676) -- 4,541 own-Whisper cues,
# the last ending at 20,093.18 s, video 23,399.666 s (header-only ffprobe of
# its playlist, 2026-09-21); and Leon Valley's 2025-06-19 meeting, whose
# transcript ends at 8,630.94 s of an 8,632.648 s video. The cue TEXT below is
# synthetic scaffolding (clean, non-repeating sentences, so the hallucination
# check stays quiet); the end times and durations are the real measurements.
# See tests/test_own_transcript_early_end.py for the rule itself and for what
# the audio after page 1676's last cue actually contains (silence).

_CLEAN_LINES = [
    "Good evening and welcome to the regular meeting of the city council.",
    "The first item tonight is the approval of the minutes from last month.",
    "Staff will now present the proposed budget amendment for the water fund.",
    "Are there any comments from the public before the council votes on this?",
]


def _clean_cues(last_end: float) -> list[dict]:
    return [
        {"start": 10.0 + 30.0 * i, "end": 30.0 + 30.0 * i, "text": t, "speaker": None}
        for i, t in enumerate(_CLEAN_LINES)
    ] + [
        {
            "start": last_end - 8.0,
            "end": last_end,
...
async def _finish_one_chunk_job(external_id: str, *, probed: float, last_end: float):
    url = f"https://example.granicus.com/player/clip/{external_id}"
    job = await crud.create_transcription_job(
        payload=_payload(f"granicus:{external_id}", url),
        input_url_normalized=url,
        requester_email=f"{external_id}@example.com",
        media_url="https://example.com/v.m3u8",
        media_kind="video",
        probed_duration_seconds=probed,
        chunk_size_seconds=int(probed) + 1,  # one chunk: the whole meeting
        skip_confirmation=True,
    )
    assert job["total_chunks"] == 1
    claim = await crud.claim_next_chunk()
    assert claim["job_id"] == job["job_id"]
    result = await crud.report_chunk_result(
        job["job_id"], success=True, shifted_segments=_clean_cues(last_end)
    )
    assert result["status"] == "completed"
    page = await crud.get_page_by_slug(job["meeting_page_slug"])
    transcribed = next(v for v in page["versions"] if v["source"] == "transcribed")
    return job, page, transcribed


async def test_completed_job_warns_when_its_own_transcript_stops_early(monkeypatch):
    monkeypatch.setenv("RTR_PARTIAL_TRANSCRIPT_CHECK", "1")
    _job, _page, version = await _finish_one_chunk_job(
        "wo935-early", probed=23_399.666, last_end=20_093.18
    )
    warnings = version["transcript_warnings"]
    assert len(warnings) == 1
    assert crud._EARLY_TRUNCATION_MARKER in warnings[0]
    assert "5 hours 35 minutes" in warnings[0]
    assert "6 hour 30 minute recording" in warnings[0]
    assert crud._has_real_warning_free_transcript(warnings) is False


async def test_completed_job_does_not_warn_when_its_transcript_reaches_the_end(
    monkeypatch,
):
    monkeypatch.setenv("RTR_PARTIAL_TRANSCRIPT_CHECK", "1")
    _job, _page, version = await _finish_one_chunk_job(
        "wo935-complete", probed=8_632.648, last_end=8_630.94
    )
    assert version["transcript_warnings"] == []


async def test_a_flagged_own_transcript_is_a_candidate_again_but_only_after_the_cooldown(
    monkeypatch,
):
    """Rule 6, confirmed against the real finder: a flagged version stops
    counting as a good transcript (so the auto-transcription finder may pick
    the page again), and WO-45's rule then makes a page whose newest job
    COMPLETED wait AUTO_TRANSCRIPTION_MAX_COOLDOWN. One repeat run per
    cooldown, not a loop."""
    from archive.db.engine import async_session
    ...
    _job, page, _version = await _finish_one_chunk_job(
        "wo935-cooldown", probed=23_399.666, last_end=20_093.18
    )
    async with async_session() as session:
        page_id = (
            await session.execute(
        ...
        assert await crud._in_auto_transcription_cooldown(session, page_id) is True
    assert crud.AUTO_TRANSCRIPTION_MAX_COOLDOWN.days == 30
