"""WO-10 (2026-08-17): archive/db/engine.py's init_models() must be a no-op
on Postgres -- Alembic (run by render.yaml's preDeployCommand) is the only
thing allowed to touch the production schema -- and must still create_all()
on SQLite, which is what every other test in this suite depends on. Pinned
by swapping the module's engine for a stub whose dialect name we control;
no real Postgres needed."""

from types import SimpleNamespace

from archive.db import engine as engine_module
