...
ATLANTA_JSON = json.dumps({
    "Event": {"EventTitle": "Committee on Council Meeting", "EventDateTimeCustomerLocal": "2026-08-03 10:00:00"},
...
MARLBOROUGH_JSON = json.dumps({
    "Event": {"EventTitle": "City Council", "EventDateTimeCustomerLocal": "2024-06-17 19:00:00"},
...
async def test_resolve_uses_direct_download_url_when_present():
    routes = {ATLANTA_API_URL: FakeResponse(status=200, text=ATLANTA_JSON, url=ATLANTA_API_URL)}
    ...
    assert result.video_url == "https://play.champds.com/DOWNLOAD-MEDIA/atlantaga/eventmainmedia/1227"
...
async def test_resolve_does_not_use_vod2_even_when_present():
    ...
    routes = {MARLBOROUGH_API_URL: FakeResponse(status=200, text=MARLBOROUGH_JSON, url=MARLBOROUGH_API_URL)}
...
async def test_resolve_returns_error_for_a_url_with_no_event_id():
    ...
    assert result.video_warnings == ["Could not find a customer/event id in this ChampDS URL."]
...
async def test_resolve_handles_api_failure_gracefully():
    ...
    assert result.video_warnings == ["Could not reach the ChampDS API for this meeting."]
...
def test_extract_jurisdiction_leaves_unrecognized_shapes_unchanged():
    result = ChampDSAssetFinder._extract_jurisdiction("Some Regional Authority", ATLANTA_URL)
