"""WO-325 (2026-09-12): passive discovery v2, group 6 (US counties that
only ever had the access-ladder sweeps, never a passive pass) of the
WO-320..325 wave (`docs/investigations/passive_discovery_full_scale.md`
and the WO-320/321/322/323/324/325 brief). Same WO-283 pipeline
(`wo283_recon.py`), run against
`research/passive_neither_6-us-counties-ladder-only.csv` (658 rows; no
rows excluded -- 0 have a bare youtube.com/youtu.be `domain` with no
path, checked directly -- see `wo331_split_population.py` in this WO's
scratch dir and this WO's BACKLOG_DONE entry), with one addition carried
from WO-283:
  ...
  - **govAccess/Akamai block.** This machine's address is blocked
    ...
    a separate `wo331_govaccess_deferred.csv` sidecar for a later run
    from a different address, once the conductor lifts the block.

Usage (from the rtr-deeplink repo root, shared venv):
    .venv/bin/python scripts/wo331_recon.py --limit 50 --concurrency 32
    .venv/bin/python scripts/wo331_recon.py --concurrency 32
    .venv/bin/python scripts/wo331_recon.py --finalize
"""
...
POPULATION_CSV = RESEARCH_DIR / "wo331_population.csv"
RECON_JSONL = RESEARCH_DIR / "wo331_recon.jsonl"
GOVACCESS_DEFERRED_CSV = RESEARCH_DIR / "wo331_govaccess_deferred.csv"

SCRATCH_DIR = Path(
    "/private/tmp/claude-501/-Users-mroconnell-Documents-rtr-deeplink--claude-worktrees-platform-detection-backfill-c9742a"
    "/1d27de13-eb07-451a-a07a-bb18900b5618/scratchpad/agents/a23aad40a6fed653c/wo331/wo331_homepages"
)
...
# WO-325: this machine's address is blocked (HTTP 403 AkamaiGHost) on every
