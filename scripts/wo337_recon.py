"""WO-337 (2026-09-12): passive discovery v2, group 6 (US counties that
...
`research/passive_neither_7-us-muni-twp-ladder-only.csv` (658 rows; no
rows excluded -- 0 have a bare youtube.com/youtu.be `domain` with no
path, checked directly -- see `wo337_split_population.py` in this WO's
...
from WO-283:
  ...
  - **govAccess/Akamai block.** This machine's address is blocked
    ...
    a separate `wo337_govaccess_deferred.csv` sidecar for a later run
    from a different address, once the conductor lifts the block.

Usage (from the rtr-deeplink repo root, shared venv):
    .venv/bin/python scripts/wo337_recon.py --limit 50 --concurrency 32
    .venv/bin/python scripts/wo337_recon.py --concurrency 32
    .venv/bin/python scripts/wo337_recon.py --finalize
"""
...
SCRATCH_DIR = Path(
    ...
    "/1d27de13-eb07-451a-a07a-bb18900b5618/scratchpad/agents/a6501ad1cc9329fa0/wo337/wo337_homepages"
)
...
# WO-337: this machine's address is blocked (HTTP 403 AkamaiGHost) on every
