"""WO-337 (2026-09-12): passive discovery v2 at full scale, phase 3 -- targeted
...
WO's own redesign calls for:
  ...
  - **Catch-all test.** One nonsense path per host
    (`/__wo337_nonsense_<random>__`), fetched once per government and
...
Output: `research/wo337_targeted.csv`, one row per (government, URL)
...
Usage (from the rtr-deeplink repo root, shared venv):
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo337_targeted_scratch.db" \\
        .venv/bin/python scripts/wo337_targeted.py --limit 50 --concurrency 32
"""
...
# WO-337 real hang, found live 2026-09-13: at --concurrency 32, a batch
# with several confidence=none governments can trigger rung 2's headless
# render (`w147.fetch_headless_sync`, Playwright/Chromium) for many of
# them at once. `page.goto(..., timeout=15000)` and `page.wait_for_
# timeout(3000)` are both bounded, but `sync_playwright()`'s own startup
# and `p.chromium.launch()` are NOT -- confirmed live: a 400-row chunk
# produced zero new rows in `wo337_targeted.csv` for 9+ minutes with the
# process's own CPU time barely moving (consistent with many concurrent
# Chromium launches jamming on this machine's resources, not a network
# stall, since a plain-HTTP GET is bounded by `GOV_TIMEOUT`). Not a
# shared-code fix (`wo147_access_ladder_sweep.py` is imported by other
# in-flight WOs) -- instead this WO's own copy caps concurrent headless
# renders to 4 at a time (HEADLESS_SEMAPHORE) and gives each one a hard
# 40s wall-clock deadline via a small dedicated thread pool
# (call_headless_with_deadline below), so a genuinely wedged launch
# times out and is recorded as an error instead of hanging the whole
# chunk. Filed in BACKLOG.md for `wo147_access_ladder_sweep.py` itself.
import threading as _threading  # noqa: E402
from concurrent.futures import ThreadPoolExecutor as _TPE, TimeoutError as _FutTimeout  # noqa: E402

HEADLESS_SEMAPHORE = _threading.Semaphore(4)
_HEADLESS_POOL = _TPE(max_workers=4)
HEADLESS_DEADLINE_SECONDS = 40


def call_headless_with_deadline(url: str):
    with HEADLESS_SEMAPHORE:
        fut = _HEADLESS_POOL.submit(_import_w147_fetch_headless_sync, url)
        try:
            return fut.result(timeout=HEADLESS_DEADLINE_SECONDS)
        except _FutTimeout:
            return None, url, f"headless-deadline-exceeded-{HEADLESS_DEADLINE_SECONDS}s"


def _import_w147_fetch_headless_sync(url: str):
    import wo147_access_ladder_sweep as _w147

    return _w147.fetch_headless_sync(url)


# WO-337 second real hang, found live 2026-09-13 (the headless fix above
# did NOT stop this one): after reducing concurrency and re-running,
# `wo337_targeted.py` still froze with zero new `wo337_targeted.csv` rows
# for 9+ minutes while the process's own CPU time kept ticking in small
# increments the whole time (`ps -o time=`, sampled every 15s -- ruled
# out a true deadlock, since a real deadlock burns ~0 CPU). `lsof -p
# <pid>` on the live process found the real cause: one TCP connection
# genuinely ESTABLISHED (not CLOSE_WAIT) to a government's own web
# server on a non-standard port, open far longer than
# `wo273_targeted.GOV_TIMEOUT` (10s) should allow. `requests`'s
# `timeout=` kwarg bounds the CONNECT phase and each individual socket
# READ, not the total time to receive a response -- a server that drips
# bytes slowly (or a response body that's simply huge, e.g. a
# accidentally-linked media file) can keep every individual read inside
# the 10s window while the overall `resp.content`/`.text` access (which
# reads the ENTIRE body) runs for however long that takes. Combined with
# `HostRateLimiter.wait_and_request()` holding its per-vendor-family lock
# for the full duration of `fn()`, one such response wedges every OTHER
# candidate sharing that same vendor-family key behind it too.
#
# Fixed here (this WO's own copy only, not the shared `wo273_targeted.
# py` -- other WOs are running against it concurrently) by giving the
# three plain-HTTP fetch call sites in this file (the catch-all
# nonsense-path probe, and fetch_and_score_v2's HEAD+GET) a real total
# cap: `stream=True` plus a hard byte ceiling AND a hard wall-clock
# ceiling enforced by actually iterating `iter_content()` ourselves,
# still going through the same `RATE_LIMITER`/`vendor_family_for_url`
# keying as `w273t.polite_fetch()` so per-vendor politeness is
# unchanged. This repo's own "never download a media file" rule is a
# second, independent reason a byte cap belongs here regardless of the
# hang -- a classified "candidate" URL is exactly the kind of link that
# could turn out to be a large audio/video file rather than an HTML
# page, and nothing upstream of this fetch already guards against that.
CAPPED_FETCH_MAX_BYTES = 3_000_000  # 3 MB -- an HTML page is never this big
CAPPED_FETCH_WALL_CLOCK_SECONDS = (
    20  # hard ceiling on top of GOV_TIMEOUT's per-read bound
)


class _CappedResponse:
    __slots__ = ("status_code", "content", "url", "_encoding")

    def __init__(self, status_code, content, url, encoding):
...
def _do_capped_request(method: str, url: str):
    resp = w273.requests.request(
        method,
        url,
        headers=w273.HEADERS,
        timeout=w273t.GOV_TIMEOUT,
        allow_redirects=True,
        stream=True,
    )
    if method.upper() == "HEAD" or resp.status_code >= 400:
        # No body to cap for HEAD; an error response is small in
        # practice and not worth an extra streaming pass.
        content = resp.content or b""
        resp.close()
        return _CappedResponse(resp.status_code, content, resp.url, resp.encoding)
    ...
    try:
        for chunk in resp.iter_content(chunk_size=65536):
            ...
            chunks.append(chunk)
            total += len(chunk)
            if total >= CAPPED_FETCH_MAX_BYTES:
                break
            if time.monotonic() - t0 >= CAPPED_FETCH_WALL_CLOCK_SECONDS:
                break
    finally:
        resp.close()
    return _CappedResponse(resp.status_code, b"".join(chunks), resp.url, resp.encoding)


def capped_polite_fetch(url: str, method: str = "GET"):
    """Drop-in replacement for `w273t.polite_fetch()` with a real total-
    time and total-bytes ceiling on top of `requests`'s own per-read
    timeout. Same rate-limiter key (vendor family) so politeness is
    unchanged."""
    key = w273.vendor_family_for_url(url)
...
PORTAL_GUARD_CSV = RESEARCH_DIR / "wo337_portal_guard_hosts.csv"

# WO-337: absolute rule for this run (repeated in the brief because WO-283
...
# WO-337 change 4: a Wistia/Vimeo/YouTube video reached through a payment,
...
def nonsense_path() -> str:
    ...
    return f"/__wo337_nonsense_{token}__"
...
def catchall_signature(domain: str) -> dict:
    ...
    try:
        resp = capped_polite_fetch(url, method="GET")
...
def load_homepage_cached(domain: str) -> dict:
    ...
    wo337_recon.jsonl for its cached homepage HTML path -- phase 3 keeps
    ...
    if _HOMEPAGE_INDEX is None:
        idx: dict[str, dict] = {}
        recon_path = RESEARCH_DIR / "wo337_recon.jsonl"
...
def fetch_and_score_v2(
    ...
    if body is not None:
        ...
        head_status = None
        try:
            head_resp = capped_polite_fetch(url, method="HEAD")
    ...
    for rank, c in enumerate(candidates[:5], start=1):
        if is_youtube_url(c["url"]):
            record_youtube_lead(
                ...
                name,
                state,
                note=f"WO-337 phase-3 candidate rank={rank} source={c.get('source', '')}",
            )
            results.append(
                {
                    "domain": domain,
...
def process_fallback_ladder(row: dict) -> list[dict]:
    ...
    if html:
        ...
        if best and is_youtube_url(best[0]):
            record_youtube_lead(
                ...
                name,
                state,
                note="WO-337 fallback-rung1-onehop best link",
            )
            best = None
    ...
    if rung == 4:
        t0 = time.monotonic()
        try:
            content, headless_final_url, herr = call_headless_with_deadline(final_url)
        except Exception as e:  # noqa: BLE001
        ...
        if content:
            ...
            if best and is_youtube_url(best[0]):
                record_youtube_lead(
                    ...
                    name,
                    state,
                    note="WO-337 fallback-rung2-headless best link",
                )
                best = None
            if best and third_party_portal_host(best[0], domain):
                record_portal_guard(
