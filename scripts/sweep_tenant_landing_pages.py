...
from scripts.build_pin_worklist import swagit_footer_name  # noqa: E402

# `reports/pin_worklist.csv`, regenerated by
# `scripts/build_pin_worklist.py` -- not the frozen copy inside a scoring
# run's report directory this script was first written against. The two
# would drift the moment a pin landed, and this script skipping a host it
# had already settled is the whole reason it reads a worklist at all.
...
# Hosts whose landing page names Google rather than a government -- the
# netloc, not the platform. A page whose VIDEO is on YouTube is still
# served by its own government's host (`www.townofrossca.gov`,
# `lasvegas.primegov.com`), and that host's landing page names the
# government perfectly well; only the four shared YouTube netlocs cannot
# be swept, and the worklist keys those by channel instead.
SKIP_HOSTS = {"youtu.be", "www.youtube.com", "youtube.com", "m.youtube.com"}
...
def candidate_names(html: str) -> List[str]:
    ...
    A Swagit footer (`build_pin_worklist.swagit_footer_name()`) comes
    first when present -- the tenant naming itself in a template every
    Swagit customer shares and nobody customizes, stronger evidence than
    anything below. Then `og:site_name` before `<h1>` before `<title>`:
    the first is an explicit statement of whose site this is, the second
    is usually the masthead, and the third is the one most likely to be
    padded with the vendor's own words. Each is also split on the common
    separators, so "Town of Lincoln | eSCRIBE Meetings" yields the half
    that is a government.
    """
    out: List[str] = []
    footer = swagit_footer_name(html)
    if footer:
        out.append(footer)
...
def best_match(names: List[str], host: str) -> Tuple[Optional[str], str, str]:
    ...
    "Plainly that government's own name" is `gov_registry.is_own_name()`,
    which this script's first run is the reason for -- see its docstring
    for the six pins it would have written without it. It lives in the
    registry package because `scripts/build_pin_worklist.py` applies the
    same rule to its proposals, and the two must not drift.
    """
    for name in names:
...
async def main() -> None:
    ...
    for row in worklist:
        ...
        if not host or host in seen:
            continue
        if host in SKIP_HOSTS or host in already:
            continue
        seen.add(host)
        hosts.append(row)
    if args.limit:
    ...
    print(f"{len(hosts)} hosts to sweep (shared YouTube and pinned hosts skipped)")
