"""WO-247 (2026-09-12): WO-235's method (PR #1000, same file copied from
`scripts/wo235_channel_pilot.py`) run at full scale. Ryan, 2026-09-12:
"Apply the method from WO-235 to the no-video-found and
meeting-without-video govs over 10k in population." Two changes from
WO-235, both Ryan's instruction, not a redesign:

  1. Population floor is 10,000, not 25,000 (`--min-population` default
     changed below; WO-235's 179-government, 25,000+ pilot population is
     a subset of this band).
  2. WO-235 additionally required a `known_platform` (a recognized
     meeting-platform hostname on file). Ryan did not ask for that filter
     here, so this band also includes governments with no suspected
     platform at all -- `_load_candidates()` below drops that condition.

A third change is mechanical, not a method change: `_load_candidates()`
now reads `rtr-business/research/jurisdiction_coverage.csv` directly
(this WO's own brief gives its header) instead of the derived
`coverage_registry.csv`, and subtracts every gov_id WO-235 already
looked at (`wo235_report.csv` + `wo235_discovery.csv` +
`wo235_decisions.csv`, unioned) so this run never re-checks a government
WO-235 already settled. Output files are this WO's own
(`wo247_discovery.csv`/`wo247_decisions.csv`/`wo247_report.csv`/
`wo247_owner_bodies.csv`) -- WO-235's own files are read-only inputs
here, never appended to.

Everything below this point -- the access ladder, the YouTube link
scanner, the yt-dlp listing call, the hand-check/identity heuristics, and
both `discover`/`finalize` stages -- is WO-235's own code, unchanged.

----- WO-235's original docstring follows, describing the method itself -----
...
Environment: run with (this WO's own file/command; see the header note
above for what actually changed from WO-235)
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo247_scratch.db" \\
    .venv/bin/python scripts/wo247_channel_band.py discover --limit 20
...
# WO-247 reads jurisdiction_coverage.csv directly (the brief's own header),
# not the derived coverage_registry.csv WO-235 used -- see the module
# docstring's "mechanical" change note above.
JURISDICTION_COVERAGE_CSV = RESEARCH_DIR / "jurisdiction_coverage.csv"
# WO-235's own output files -- read-only inputs here, used only to
# subtract governments it already checked. Never appended to.
...
DISCOVERY_CSV = RESEARCH_DIR / "wo247_discovery.csv"
DECISIONS_CSV = RESEARCH_DIR / "wo247_decisions.csv"
REPORT_CSV = RESEARCH_DIR / "wo247_report.csv"
OWNER_BODIES_CSV = RESEARCH_DIR / "wo247_owner_bodies.csv"
HEADLESS_BUDGET_STATE = RESEARCH_DIR / "wo247_headless_budget_used.txt"
...
def _wo235_checked_gov_ids() -> set:
    """Every gov_id WO-235 looked at (report + discovery + decisions,
    unioned) -- this WO never re-checks one of these (brief's own
    subtraction rule)."""
    checked: set = set()
    for path in (WO235_REPORT_CSV, WO235_DISCOVERY_CSV, WO235_DECISIONS_CSV):
        if not path.exists():
            continue
...
def _load_candidates(min_population: float = 10000.0) -> List[dict]:
    ...
    is a lead, re-derive it" rule. Reads the research CSV directly (this
    WO's own brief gives its header), not the derived coverage_registry.csv
    WO-235 used -- and, unlike WO-235, does NOT require a known
    meeting-platform on file (Ryan's instruction for this band explicitly
    didn't filter on that). Row shape is mapped to the internal fields the
    rest of this script (copied from WO-235, unchanged below this
    function) expects: gov_id, name, state, population, known_platform
    (display/reporting only here, not a filter), domain, hub_url."""
    checked = _wo235_checked_gov_ids()
    rows: List[dict] = []
    with JURISDICTION_COVERAGE_CSV.open(newline="", encoding="utf-8") as f:
        for row in csv.DictReader(f):
            ...
            known_platform = (
                (row.get("suspected_meeting_link_provider") or "").strip()
        ...
        if result.segments:
            ...
            if channel_owner == "own" and handle:
                queue_probe.write_pin_row(
                    ...
                    gov_id=gov_id,
                    strength="fallback",
                    source="wo247_channel_band",
                    evidence=f"{name}, {state} -- WO-247 channel band, gov_id={gov_id}",
                )
            else:
                wo134.maybe_write_tenant_override(
                    "youtube", result, video_url, gov_id, name, "wo247_channel_band"
                )
...
def main() -> None:
    ...
    p_discover.add_argument("--min-population", type=float, default=10000.0)
