"""WO-338 (2026-09-13): phase 1 (reconnaissance), rerun-with-fixed-verifier
pass, restricted to the 646 of 2,825 in-scope governments
(`research/passive_rerun_1-verifier-fix.csv`, filtered to
`prior_reject_reason != no-platform-link-found`) whose `recon_file`
column is blank or names the v1-shaped `research/wo273_recon.jsonl`
(see the WO-337/338 addendum). The other 2,179 rows already have a
v2-shaped phase-1 record on disk and are loaded directly by
`wo338_merge_recon.py`, never re-fetched. Population:
`research/wo338_recon_needed.csv`. Same WO-283 pipeline
(`wo283_recon.py`), with one addition carried from WO-283/WO-325:
  ...
  - **govAccess/Akamai block.** This machine's address is blocked
    ...
    a separate `wo338_govaccess_deferred.csv` sidecar for a later run
    from a different address, once the conductor lifts the block.

Usage (from the rtr-deeplink repo root, shared venv):
    .venv/bin/python scripts/wo338_recon.py --limit 50 --concurrency 32
    .venv/bin/python scripts/wo338_recon.py --concurrency 32
    .venv/bin/python scripts/wo338_recon.py --finalize
"""
...
POPULATION_CSV = RESEARCH_DIR / "wo338_recon_needed.csv"
RECON_JSONL = RESEARCH_DIR / "wo338_recon_fresh.jsonl"
GOVACCESS_DEFERRED_CSV = RESEARCH_DIR / "wo338_govaccess_deferred.csv"

SCRATCH_DIR = Path(
    "/private/tmp/claude-501/-Users-mroconnell-Documents-rtr-deeplink--claude-worktrees-platform-detection-backfill-c9742a"
    "/1d27de13-eb07-451a-a07a-bb18900b5618/scratchpad/agents/a53045534b063f820/wo338/wo338_homepages"
)
...
# WO-338: this machine's address is blocked (HTTP 403 AkamaiGHost) on every
