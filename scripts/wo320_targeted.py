"""WO-320 (2026-09-12): passive discovery v2 at full scale, phase 3 -- targeted
...
WO's own redesign calls for:
  ...
  - **Catch-all test.** One nonsense path per host
    (`/__wo320_nonsense_<random>__`), fetched once per government and
...
Output: `research/wo320_targeted.csv`, one row per (government, URL)
...
Usage (from the rtr-deeplink repo root, shared venv):
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo320_targeted_scratch.db" \\
        .venv/bin/python scripts/wo320_targeted.py --limit 50 --concurrency 32
"""
...
PORTAL_GUARD_CSV = RESEARCH_DIR / "wo320_portal_guard_hosts.csv"
...
# WO-320 change 4: a Wistia/Vimeo/YouTube video reached through a payment,
...
def youtube_host(url: str) -> str:
    ...
    WO-320's launch instructions are literal: never fetch a youtube.com
...
def nonsense_path() -> str:
    ...
    return f"/__wo320_nonsense_{token}__"
...
def load_homepage_cached(domain: str) -> dict:
    ...
    wo320_recon.jsonl for its cached homepage HTML path -- phase 3 keeps
    ...
    if _HOMEPAGE_INDEX is None:
        idx: dict[str, dict] = {}
        recon_path = RESEARCH_DIR / "wo320_recon.jsonl"
