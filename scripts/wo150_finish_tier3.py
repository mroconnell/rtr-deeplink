...
`app/platforms/queue_probe.py`'s DEFAULT_SIDECAR_PATH,
scripts/tier3_auto_transcription_queue_probe.csv):
  ...
  3. Append the ProbeResult to the shared sidecar (append_probe_row) --
     the same file scripts/probe_tier3_queue.py and
     scripts/feed_tier3_auto_transcription.py already write to, so a
     later run/reader sees one consistent probe history.
  4. On verdict == "accept": append the meeting URL to the real
     scripts/tier3_auto_transcription_queue.txt (dedupe-checked against
     _existing_tier3_queue_urls()) and write the row's pin (if any) to
     tenant_overrides.csv (dedupe-checked, same as the sweep's own pin
     writer).
  5. On "reject-dead"/"reject-short": never touch the real queue file.

Writes rtr-business/research/wo150_tier3_pending.csv's own probe verdict
back into a copy (wo150_tier3_pending.csv is left as the sweep's
original append-only record; this script's own per-row outcome is
logged to wo150_tier3_finish_log.csv) so the funnel can show
"rejected by probe" as its own row, per this WO's report requirements.
...
def _write_pin(pin_row: str) -> bool:
    """pin_row is 'tenant_host=X;match=Y;gov_id=Z;strength=fallback;source=...'."""
    if not pin_row:
        return False
