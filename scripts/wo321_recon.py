"""WO-321 (2026-09-12): passive discovery v2, group 2 (US counties) of the
"received neither pass" population Breadth split out for the WO-320..323
parallel wave (`docs/investigations/passive_discovery_full_scale.md` and
the WO-320/321/322/323 brief). Same WO-283 pipeline
(`wo283_recon.py`), run against
`research/passive_neither_2-us-counties.csv` (223 rows; 220 after
excluding 3 rows whose `domain` is a bare youtube.com/youtu.be host with
no path -- see `wo321_split_population.py` in this WO's scratch dir and
this WO's BACKLOG_DONE entry), with one addition carried from WO-283:
  ...
  - **govAccess/Akamai block.** This machine's address is blocked
    ...
    a separate `wo321_govaccess_deferred.csv` sidecar for a later run
    from a different address, once the conductor lifts the block.

Usage (from the rtr-deeplink repo root, shared venv):
    .venv/bin/python scripts/wo321_recon.py --limit 50 --concurrency 32
    .venv/bin/python scripts/wo321_recon.py --concurrency 32
    .venv/bin/python scripts/wo321_recon.py --finalize
"""
...
POPULATION_CSV = RESEARCH_DIR / "wo321_population.csv"
RECON_JSONL = RESEARCH_DIR / "wo321_recon.jsonl"
GOVACCESS_DEFERRED_CSV = RESEARCH_DIR / "wo321_govaccess_deferred.csv"

SCRATCH_DIR = Path(
    "/private/tmp/claude-501/-Users-mroconnell-Documents-rtr-deeplink--claude-worktrees-platform-detection-backfill-c9742a"
    "/1d27de13-eb07-451a-a07a-bb18900b5618/scratchpad/agents/adcf42836334c3c83/wo321/wo321_homepages"
)
...
# WO-321: this machine's address is blocked (HTTP 403 AkamaiGHost) on every
