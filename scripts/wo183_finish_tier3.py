#!/usr/bin/env python3
"""WO-183: probe every tier-3 candidate `wo183_access_ladder_sweep.py`
...
Copied from `scripts/wo191_access_ladder_sweep.py`'s sibling
`wo191_finish_tier3.py` (WO-191's own copy, still uncommitted in its own
in-progress worktree, so a module import wasn't possible) rather than
imported, per this WO's own instructions. Only the file-path constants
and this docstring changed.

Reads `rtr-business/research/wo183_tier3_pending.csv` (one row per
candidate: gov_id, platform, meeting_url, video_url, source_url,
jurisdiction, pin_row). For each row not already probed (checked against
...
Writes `rtr-business/research/wo183_tier3_finish_log.csv` -- one row per
candidate with the probe verdict, so a later apply step can tell "queued
for real" (probe accept) apart from "found, but rejected by the probe"
(reject-dead/reject-short).
...
Usage:
    python scripts/wo183_finish_tier3.py
    python scripts/wo183_finish_tier3.py --limit 25
"""
...
PENDING_CSV = RESEARCH_DIR / "wo183_tier3_pending.csv"
FINISH_LOG_CSV = RESEARCH_DIR / "wo183_tier3_finish_log.csv"
FINISH_LOG_FIELDS = [
    "gov_id",
    "platform",
    "meeting_url",
        ...
        writer.writerow(
            {
                ...
                "source": source or "wo183_access_ladder_sweep",
                "evidence": evidence or f"WO-183 tier-3 probe accept, gov_id={gov_id}",
            }
        )
    return True
