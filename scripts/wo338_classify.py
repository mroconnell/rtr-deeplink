"""WO-338 (2026-09-12): passive discovery v2 at full scale, phase 2 -- offline
classification over phase 1's raw recon file (`wo338_recon.jsonl`).
...
Output: `research/wo338_classified.csv`, one row per government, with a
...
Usage (from the rtr-deeplink repo root, shared venv):
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo338_classify_scratch.db" \\
        .venv/bin/python scripts/wo338_classify.py
...
RECON_JSONL = RESEARCH_DIR / "wo338_recon.jsonl"
...
def classify_record(rec: dict) -> dict:
    ...
    # WO-338 caution 1: for this population, `domain` is sometimes itself a
