...
rtr-business/research/nationwide_1911_confirmed_hits.csv.

Third batch, same shape and same pipeline as the first two runs
(scripts/nationwide_395_ingest.py, scripts/nationwide_431_ingest.py, both
kept as-is for their own logs' resume behavior) -- 486 MORE governments,
same exact column shape (gov_id, unit_name, population, homepage,
homepage_status, hop2_pages_checked, hop2_urls, platform_hits,
hit_source_urls, detail), newly discovered via a state-directory/
Municipal-League sweep (all 50 states + DC, including a 14-state League
fallback pass) and confirmed via the same two-hop platform scan, never
run through this pipeline before. This batch skews toward some large,
real cities that were never covered before -- California League data
surfaced Oxnard, Oceanside, Sunnyvale, Fairfield, South Gate, Tustin,
Bellflower, all genuinely new and all in the top 10 of this batch by
population -- handled like any other row, no special-casing, just worth
knowing since a bad ingest on a big city is more visible than a small
town. All real bugs the first two runs found and fixed are carried over
...
`;id=...` param). See that file's original docstring/comments (preserved
below) for the full reasoning; only the input/output paths differ.
...
Writes a per-row CSV log to rtr-business/research/nationwide_1911_ingest_log.csv
(resumable: rows already present for a given gov_id are skipped on a
re-run) and prints a final tally.

Usage (from repo root, venv active):
    python scripts/nationwide_1911_ingest.py
    python scripts/nationwide_1911_ingest.py --limit 20        # smoke test
    python scripts/nationwide_1911_ingest.py --start-after us:county:53011
"""
...
INPUT_CSV = RESEARCH_DIR / "nationwide_1911_confirmed_hits.csv"
LOG_CSV = RESEARCH_DIR / "nationwide_1911_ingest_log.csv"
