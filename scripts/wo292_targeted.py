"""WO-292 (2026-09-12): school-district pilot, phase 3 -- targeted
...
Copied from `wo282_targeted.py` and adapted for school sites in three
places: (1) `CLASSIFIED_CSV`/`TARGETED_CSV` point at the WO-292 files;
(2) the fallback-ladder rescoring (`score_raw_link`/
`best_link_even_below_threshold`) is now gov_id-aware, so a us:sd: row
gets the WO-292 school vocabulary added on top of the city/county one,
same as phase 2; (3) a candidate phase 2 already tagged
`source_kind="agenda_only"` (BoardDocs/Simbli) is recorded but never
fetched here -- per the brief, agenda-only platforms are reported, not
chased for video, so spending a real HTTP request confirming a page this
pipeline will never ingest from would just be noise.
...
def load_homepage_cached(domain: str) -> dict:
    ...
    if _HOMEPAGE_INDEX is None:
        idx: dict[str, dict] = {}
        recon_path = RESEARCH_DIR / "wo292_recon.jsonl"
...
def score_raw_link(
    ...
    if every link scores zero or negative. WO-292: `gov_id` selects the
    school vocabulary addition for a us:sd: row via w147._weights_for_gov
    (the same lookup find_hop_links() now uses), so the fallback ladder's
    own scoring stays consistent with phase 2's for a school district."""
    netloc = urlparse(full_url).netloc.lower()
    hay = f"{text} {href}".lower()
    path = urlparse(full_url).path
    path_tokens = w147._hop_tokenize(path)
    anchor_words = w147._hop_tokenize(text)
    path_weights, anchor_weights = w147._weights_for_gov(gov_id)
    path_score = sum(path_weights.get(t, 0.0) for t in set(path_tokens))
    path_score += sum(
        path_weights.get(b, 0.0) for b in set(w147._hop_bigrams(path_tokens))
    )
    anchor_score = sum(anchor_weights.get(w, 0.0) for w in set(anchor_words))
    target_bonus = 0.0
    is_vendor = (
...
def best_link_even_below_threshold(
    html: str, final_url: str, *, gov_id: str = ""
    ...
    for a in soup.find_all("a", href=True):
        ...
        score = score_raw_link(text, href, full, base_netloc, a, gov_id=gov_id)
        if score > best_score:
...
def process_with_candidates(row: dict) -> list[dict]:
    ...
    for rank, c in enumerate(candidates[:5], start=1):
        if c.get("kind") == "agenda_only":
            # WO-292: BoardDocs/Simbli -- recorded, never fetched (see
            ...
            results.append(
                {
                    ...
                    "catchall_confirmed": False,
                    "platform_confirmed": "",
                    "platform_signal": c.get("agenda_only_platform", ""),
                    "name_match": False,
                    "fallback_rung": 0,
                    "error": "agenda-only-not-fetched",
                    "timing_ms": 0,
                }
            )
            continue
...
def process_fallback_ladder(row: dict) -> list[dict]:
    ...
    if html:
        best = best_link_even_below_threshold(html, final_url, gov_id=gov_id)
        if best:
            url, score = best
            t0 = time.monotonic()
    ...
    if rung == 4:
        ...
        if content:
            best = best_link_even_below_threshold(
                content, headless_final_url, gov_id=gov_id
            )
            if best:
                url, score = best
                t1 = time.monotonic()
