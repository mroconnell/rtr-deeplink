"""WO-283 (2026-09-12): passive discovery v2 at full scale, phase 1 --
...
reach, with one addition:
  ...
  - **govAccess/Akamai block.** This machine's address is blocked
    ...
    a separate `wo283_govaccess_deferred.csv` sidecar for a later run
    from a different address, once the conductor lifts the block.

Usage (from the rtr-deeplink repo root, shared venv):
    .venv/bin/python scripts/wo283_recon.py --limit 50 --concurrency 32
    .venv/bin/python scripts/wo283_recon.py --concurrency 32
    .venv/bin/python scripts/wo283_recon.py --finalize
"""
...
POPULATION_CSV = RESEARCH_DIR / "wo283_population.csv"
RECON_JSONL = RESEARCH_DIR / "wo283_recon.jsonl"
GOVACCESS_DEFERRED_CSV = RESEARCH_DIR / "wo283_govaccess_deferred.csv"

SCRATCH_DIR = Path(
    "/private/tmp/claude-501/-Users-mroconnell-Documents-rtr-deeplink--claude-worktrees-platform-detection-backfill-c9742a"
    "/1d27de13-eb07-451a-a07a-bb18900b5618/scratchpad/agents/a309fabe0ff1fa3d2/wo283_homepages"
)
...
# WO-283: this machine's address is blocked (HTTP 403 AkamaiGHost) on every
