"""WO-358: hand-check driver for the tier-1/tier-3 candidates from
`research/wo358_verify.csv`. Resolves each candidate's own meeting_url a
second time via `resolve_via_platform()` (never a youtube.com/youtu.be
URL -- none of these rows are YouTube, those are tier-2 leads handled
separately), runs `classify_video_hand_check()` as the automatic
pre-filter, and prints title/date/jurisdiction/segments for a human read
against the government's own name+state, per CLAUDE.md's hand-check rule
and the county-tenant/cross-border-namesake cautions WO-350 found on
these same four platforms.

Usage:
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo358_handcheck.db" \\
        .venv/bin/python scripts/wo358_handcheck.py
"""
...
OUT_CSV = RESEARCH_DIR / "wo358_handcheck.csv"
