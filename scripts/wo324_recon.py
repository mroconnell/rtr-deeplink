"""WO-324 (2026-09-12): passive discovery v2, group 2 (US counties) of the
...
no path -- see `wo324_split_population.py` in this WO's scratch dir and
this WO's BACKLOG_DONE entry), with one addition carried from WO-283:
  ...
  - **govAccess/Akamai block.** This machine's address is blocked
    ...
    a separate `wo324_govaccess_deferred.csv` sidecar for a later run
    from a different address, once the conductor lifts the block.

Usage (from the rtr-deeplink repo root, shared venv):
    .venv/bin/python scripts/wo324_recon.py --limit 50 --concurrency 32
    .venv/bin/python scripts/wo324_recon.py --concurrency 32
    .venv/bin/python scripts/wo324_recon.py --finalize
"""
...
GOVACCESS_DEFERRED_CSV = RESEARCH_DIR / "wo324_govaccess_deferred.csv"

SCRATCH_DIR = Path(
    "/private/tmp/claude-501/-Users-mroconnell-Documents-rtr-deeplink--claude-worktrees-platform-detection-backfill-c9742a"
    "/1d27de13-eb07-451a-a07a-bb18900b5618/scratchpad/agents/a5ec6b6cf78e0d437/wo324/wo324_homepages"
)
...
# WO-324: this machine's address is blocked (HTTP 403 AkamaiGHost) on every
