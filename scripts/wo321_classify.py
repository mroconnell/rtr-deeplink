"""WO-321 (2026-09-12): passive discovery v2 at full scale, phase 2 -- offline
classification over phase 1's raw recon file (`wo321_recon.jsonl`).
...
Output: `research/wo321_classified.csv`, one row per government, with a
...
Usage (from the rtr-deeplink repo root, shared venv):
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo321_classify_scratch.db" \\
        .venv/bin/python scripts/wo321_classify.py
...
def classify_record(rec: dict) -> dict:
    ...
    # WO-321 caution 1: for this population, `domain` is sometimes itself a
