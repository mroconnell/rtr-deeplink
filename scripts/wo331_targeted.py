...
WO's own redesign calls for:
  ...
  - **Catch-all test.** One nonsense path per host
    (`/__wo331_nonsense_<random>__`), fetched once per government and
...
Output: `research/wo331_targeted.csv`, one row per (government, URL)
...
Usage (from the rtr-deeplink repo root, shared venv):
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo331_targeted_scratch.db" \\
        .venv/bin/python scripts/wo331_targeted.py --limit 50 --concurrency 32
"""
...
CLASSIFIED_CSV = RESEARCH_DIR / "wo331_classified.csv"
TARGETED_CSV = RESEARCH_DIR / "wo331_targeted.csv"
PORTAL_GUARD_CSV = RESEARCH_DIR / "wo331_portal_guard_hosts.csv"
...
def nonsense_path() -> str:
    ...
    return f"/__wo331_nonsense_{token}__"
...
def load_homepage_cached(domain: str) -> dict:
    ...
    wo331_recon.jsonl for its cached homepage HTML path -- phase 3 keeps
    ...
    if _HOMEPAGE_INDEX is None:
        idx: dict[str, dict] = {}
        recon_path = RESEARCH_DIR / "wo331_recon.jsonl"
