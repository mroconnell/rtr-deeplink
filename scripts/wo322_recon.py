"""WO-322 (2026-09-12): passive discovery v2 at full scale, phase 1 --
...
reach, with one addition:
  ...
  - **govAccess/Akamai block.** This machine's address is blocked
    ...
    a separate `wo322_govaccess_deferred.csv` sidecar for a later run
    from a different address, once the conductor lifts the block.

Usage (from the rtr-deeplink repo root, shared venv):
    .venv/bin/python scripts/wo322_recon.py --limit 50 --concurrency 32
    .venv/bin/python scripts/wo322_recon.py --concurrency 32
    .venv/bin/python scripts/wo322_recon.py --finalize
"""
...
RECON_JSONL = RESEARCH_DIR / "wo322_recon.jsonl"
GOVACCESS_DEFERRED_CSV = RESEARCH_DIR / "wo322_govaccess_deferred.csv"

SCRATCH_DIR = Path(
    "/private/tmp/claude-501/-Users-mroconnell-Documents-rtr-deeplink--claude-worktrees-platform-detection-backfill-c9742a"
    "/1d27de13-eb07-451a-a07a-bb18900b5618/scratchpad/agents/ada354a2995529424/wo322/homepages"
)
...
# WO-322: this machine's address is blocked (HTTP 403 AkamaiGHost) on every
...
def maybe_refresh_cdx_health() -> bool:
    """WO-322 fix: a real stall was hit live during this WO's own run
    (concurrency=48, limit=400) -- the jsonl file stopped growing at
    399/400 with zero CPU and zero open connections, for 13+ minutes,
    consistent with one thread holding `_cdx_health_lock` across the
    actual `probe_cdx_health()` network call while every other
    government's own call into this function (unconditional, at the top
    of `process_government_v2`) blocked on the same lock waiting to even
    increment its counter. The lock now only guards the counter/flag,
    never the network call itself, so one slow health probe can no
    longer stall every other in-flight government."""
    global _cdx_healthy, _gov_count_since_health_check
    with _cdx_health_lock:
        _gov_count_since_health_check += 1
        count = _gov_count_since_health_check
        should_check = count == 1 or (count % CDX_HEALTH_EVERY == 0)
        current = _cdx_healthy
    if not should_check:
        return current
    healthy = probe_cdx_health()
    with _cdx_health_lock:
        _cdx_healthy = healthy
    log(f"[cdx-health] probe #{count}: {'healthy' if healthy else 'UNHEALTHY'}")
    return healthy
