#!/usr/bin/env python3
"""WO-283: probe every tier-3 candidate `wo283_targeted.py`
...
`rtr-business/research/wo283_tier3_pending.csv` (one row per candidate:
...
Writes `rtr-business/research/wo283_tier3_finish_log.csv` -- one row per
candidate with the probe verdict, so `wo283_apply_to_jc.py` can tell
...
Usage:
    python scripts/wo283_finish_tier3.py
    python scripts/wo283_finish_tier3.py --limit 25
"""
...
PENDING_CSV = RESEARCH_DIR / "wo283_tier3_pending.csv"
FINISH_LOG_CSV = RESEARCH_DIR / "wo283_tier3_finish_log.csv"
FINISH_LOG_FIELDS = [
    "gov_id",
    "platform",
    "meeting_url",
