"""WO-191 (2026-09-11): first-pass access-ladder sweep of never-tested
governments in rtr-business/research/jurisdiction_coverage.csv --
candidate list in research/wo191_candidates.csv (built by
scripts/wo191_build_candidates.py), population descending, unknown
population placed between the 5,000+ US rows and the under-1,000 rows
per docs/BREADTH_SWEEP_BRIEF.md's priority order.
...
Reused BY IMPORT, never copied or edited, per this WO's own instructions:
  ...
  - `scripts/wo134_confirmed_hits_ingest.py`'s `process_row()` for the
    ...
    exactly like WO-147/WO-150/WO-181 before it.

Headless render budget: 600 for this WHOLE run (not per session), a
running total under `research/wo191_headless_budget.json` so it holds
across a resumed run. `wo147.fetch_headless` is monkeypatched (a runtime
wrapper around the SAME function, not an edit to that file) to enforce
this budget -- once exhausted, headless simply reports "no page" instead
of launching a browser, which folls back to a `no-platform-link-found`
content reject for that host (the existing behavior for "headless found
nothing").

Discovery-ledger enrichment (rtr-discovery's own enumerator, the
"platform API" step WO-147 layered on top of the plain ladder) is
DELIBERATELY NOT used here: WO-147's own scratch-ledger copy
(/tmp/wo147_ledger.db) is a single sqlite file, and with several other
sessions running the same kind of sweep concurrently tonight (WO-184,
WO-189, WO-190), sharing that file across independent processes risks a
real write collision this WO doesn't need to take on -- these are
never-tested rows with no confirmed platform signature at all in the
overwhelming majority of cases, so a bare (platform, seed) hit_source_url
pair (already handled by wo134.process_row -- it walks up to 6 candidate
URLs on its own, calling resolve_seed()) covers the "known_platform"
rung 1 case just as well without touching the shared ledger.
...
Usage:
    python scripts/export_meeting_inventory.py --out-dir /tmp/wo191_inventory --source export
    python scripts/wo191_build_candidates.py
    python scripts/wo191_access_ladder_sweep.py --limit 20   # smoke test
    python scripts/wo191_access_ladder_sweep.py              # full run, resumable
"""
...
CANDIDATES_CSV = RESEARCH_DIR / "wo191_candidates.csv"
REPORT_CSV = RESEARCH_DIR / "wo191_report.csv"
DISCOVERY_SEEDS_CSV = RESEARCH_DIR / "wo191_discovery_seeds.csv"
HOST_ACCESS_MODES_CSV = RESEARCH_DIR / "wo191_host_access_modes.csv"
TIER3_PENDING_CSV = RESEARCH_DIR / "wo191_tier3_pending.csv"
HEADLESS_BUDGET_JSON = RESEARCH_DIR / "wo191_headless_budget.json"
DEFAULT_INVENTORY_CSV = Path("/tmp/wo191_inventory/meeting_inventory.csv")
...
async def _budgeted_fetch_headless(
    ...
    if _headless_used >= HEADLESS_BUDGET_TOTAL:
        return (
            None,
            url,
            f"wo191 headless budget exhausted ({HEADLESS_BUDGET_TOTAL} used)",
        )
    _headless_used += 1
...
def tier3_pending_handler(
    *, gov_id, unit_name, platform, final_seed, hit_url, title, date, result
    ...
    if platform in wo134.SHARED_HOST_PLATFORMS:
        ...
        if host and match:
            pin_row = (
                f"{host}|{match}|{gov_id}|fallback|wo191_access_ladder_sweep|"
                f"{unit_name} -- WO-191 tier-3 pending, gov_id={gov_id}"
            )
...
async def process_candidate(
    ...
    try:
        result = await wo134.process_row(
            session, synthetic_row, covered_gov_ids, "wo191_access_ladder_sweep"
        )
