"""WO-325 (2026-09-12): passive discovery v2 at full scale, phase 2 -- offline
classification over phase 1's raw recon file (`wo325_recon.jsonl`).
...
Output: `research/wo325_classified.csv`, one row per government, with a
...
Usage (from the rtr-deeplink repo root, shared venv):
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo325_classify_scratch.db" \\
        .venv/bin/python scripts/wo325_classify.py
...
def classify_record(rec: dict) -> dict:
    ...
    # WO-325 caution 1: for this population, `domain` is sometimes itself a
    # vendor meeting-platform tenant host, not the government's own site
    # (e.g. `bedfordoh.primegov.com`, `denver.granicus.com`,
    # `charlestonwv.portal.civicclerk.com`) -- the domain root IS the
    # meeting hub, so this is confirmed platform=high confidence
