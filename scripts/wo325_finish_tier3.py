#!/usr/bin/env python3
"""WO-325: probe every tier-3 candidate `wo325_targeted.py`
...
`rtr-business/research/wo325_tier3_pending.csv` (one row per candidate:
...
Writes `rtr-business/research/wo325_tier3_finish_log.csv` -- one row per
candidate with the probe verdict, so `wo325_apply_to_jc.py` can tell
...
Usage:
    python scripts/wo325_finish_tier3.py
    python scripts/wo325_finish_tier3.py --limit 25
"""
...
PENDING_CSV = RESEARCH_DIR / "wo325_tier3_pending.csv"
FINISH_LOG_CSV = RESEARCH_DIR / "wo325_tier3_finish_log.csv"
FINISH_LOG_FIELDS = [
    "gov_id",
    "platform",
    "meeting_url",
