"""WO-320 (2026-09-12): passive discovery v2 at full scale, phase 2 -- offline
classification over phase 1's raw recon file (`wo320_recon.jsonl`).
...
WO-320 caution 1: this population's `domain` column is sometimes a
...
Output: `research/wo320_classified.csv`, one row per government, with a
...
Usage (from the rtr-deeplink repo root, shared venv):
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo320_classify_scratch.db" \\
        .venv/bin/python scripts/wo320_classify.py
...
def classify_record(rec: dict) -> dict:
    ...
    # WO-320 caution 1: the domain COLUMN itself may already be a vendor
