"""WO-323 (2026-09-12): passive discovery v2 at full scale, phase 2 -- offline
classification over phase 1's raw recon file (`wo323_recon.jsonl`).
...
WO-323 caution 1: this population's `domain` column is sometimes a
vendor tenant HOST itself (e.g. `pub-lincoln.escribemeetings.com`), not
the government's own site -- for those rows the domain root IS the
meeting hub. `vendor_platform_for_url()` already recognizes the host by
substring regardless of who links to it, but relying on that alone means
a row where the homepage fetch failed (no HTML, no extracted links)
would fall through to phase 3's fallback ladder (headless render,
AgendaCenter probes) instead of straight to the real hub. `classify_
record()` below checks the domain root itself first and, when it's a
recognized vendor host, seeds it directly into `all_candidates` as a
`kind=platform`, `source=vendor-host-domain` top candidate -- independent
of whether phase 1's homepage fetch produced anything at all.

Output: `research/wo323_classified.csv`, one row per government, with a
...
Usage (from the rtr-deeplink repo root, shared venv):
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo323_classify_scratch.db" \\
        .venv/bin/python scripts/wo323_classify.py
...
def classify_record(rec: dict) -> dict:
    ...
    # WO-323 caution 1: the domain COLUMN itself may already be a vendor
    # tenant host -- seed its root URL as a top candidate regardless of
    # whether phase 1's homepage fetch produced any links to find it
    # through.
