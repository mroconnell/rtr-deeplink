"""WO-357: hand-check driver for the tier1/tier3 candidates from
`research/wo357_verify.csv` (the CivicPlus label-only bucket -- WO-349's
own bucket B, 234 governments with a platform label but no confirmed
tenant URL). Resolves each candidate's own meeting_url via
`resolve_via_platform()` (never a youtube.com/youtu.be URL -- the tier-2
YouTube leads are excluded here on purpose, per the standing "never
fetch YouTube" rule; they go straight to youtube_channel_leads.csv
unverified), runs `classify_video_hand_check()` as the automatic
pre-filter, and prints title/date/jurisdiction/segments for a human read
against the government's own name+state.

Usage:
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo357_handcheck.db" \\
        .venv/bin/python scripts/wo357_handcheck.py
"""
...
OUT_CSV = RESEARCH_DIR / "wo357_handcheck.csv"
