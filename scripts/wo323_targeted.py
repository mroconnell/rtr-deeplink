"""WO-323 (2026-09-12): passive discovery v2 at full scale, phase 3 -- targeted
...
WO's own redesign calls for:
  ...
  - **Catch-all test.** One nonsense path per host
    (`/__wo323_nonsense_<random>__`), fetched once per government and
...
Output: `research/wo323_targeted.csv`, one row per (government, URL)
...
Usage (from the rtr-deeplink repo root, shared venv):
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo323_targeted_scratch.db" \\
        .venv/bin/python scripts/wo323_targeted.py --limit 50 --concurrency 32
"""
...
CLASSIFIED_CSV = RESEARCH_DIR / "wo323_classified.csv"
TARGETED_CSV = RESEARCH_DIR / "wo323_targeted.csv"
PORTAL_GUARD_CSV = RESEARCH_DIR / "wo323_portal_guard_hosts.csv"
...
# WO-323 change 4: a Wistia/Vimeo/YouTube video reached through a payment,
...
def youtube_host(url: str) -> str:
    """Returns 'youtube' if `url` is a youtube.com/youtu.be URL, else ''.
    WO-323's launch instructions are literal: never fetch a youtube.com
    or youtu.be URL for any reason, not even a plain candidate-fetch.
    Called BEFORE any fetch_and_score_v2() call, so this stays a pure
    string/urlparse check with zero network I/O."""
    netloc = urlparse(url).netloc.lower()
...
_youtube_lead_write_lock = threading.Lock()
_youtube_leads_seen: set[tuple] | None = None
_youtube_pinned_handles: set | None = None
TENANT_OVERRIDES_CSV = (
    REPO_ROOT / "app" / "utils" / "jurisdiction_data" / "tenant_overrides.csv"
)


def _load_youtube_pinned_handles() -> set:
    """@handles and bare video ids already pinned for youtube.com in
    tenant_overrides.csv -- checked before writing a new lead row, per
    the brief's 'dedupe against the file and against pins first'."""
    global _youtube_pinned_handles
    if _youtube_pinned_handles is not None:
        return _youtube_pinned_handles
    pinned = set()
    if TENANT_OVERRIDES_CSV.exists():
        with open(TENANT_OVERRIDES_CSV, newline="", encoding="utf-8") as f:
            for row in csv.DictReader(f):
                if (row.get("tenant_host") or "").strip().lower() == "youtube.com":
                    match = (row.get("match") or "").strip()
                    if match.startswith("channel="):
                        pinned.add(match[len("channel=") :].lstrip("@").lower())
                    elif match:
                        pinned.add(match.lower())
    _youtube_pinned_handles = pinned
    return pinned


def _youtube_key_from_url(url: str) -> str:
    """Best-effort @handle or video-id extraction from a youtube URL, for
    the pin dedupe check -- not a parse, just enough to compare."""
    m = re.search(r"/@([\w.-]+)", url)
    if m:
...
def _load_youtube_leads_seen() -> set:
    global _youtube_leads_seen
    if _youtube_leads_seen is not None:
        return _youtube_leads_seen
    seen = set()
    if YOUTUBE_LEADS_CSV.exists():
        with open(YOUTUBE_LEADS_CSV, newline="", encoding="utf-8") as f:
            lines = [ln for ln in f if not ln.startswith("#")]
        for row in csv.DictReader(lines):
            seen.add((row.get("channel_url", ""), row.get("gov_id", "")))
    _youtube_leads_seen = seen
    return seen
...
def record_youtube_lead(
    ...
    key = (url, gov_id)
    with _youtube_lead_write_lock:
        pin_key = _youtube_key_from_url(url)
        if pin_key and pin_key in _load_youtube_pinned_handles():
            return
        seen = _load_youtube_leads_seen()
        if key in seen:
            return
        seen.add(key)
        write_header = not YOUTUBE_LEADS_CSV.exists()
        with open(YOUTUBE_LEADS_CSV, "a", newline="", encoding="utf-8") as f:
            w = csv.DictWriter(
                f,
                fieldnames=[
                    "channel_url",
            ...
            w.writerow(
                {
                    ...
                    "kind": kind,
                    "verified": "false",
                    "note": "not fetched -- WO-323's no-YouTube-calls rule; URL-shape kind guess only",
                }
            )


def _youtube_skip_result(url: str, extra: dict) -> dict:
    base = {
        "fetch_method": "",
        "http_status": None,
        "body_len": 0,
...
def nonsense_path() -> str:
    ...
    return f"/__wo323_nonsense_{token}__"
...
def load_homepage_cached(domain: str) -> dict:
    ...
    wo323_recon.jsonl for its cached homepage HTML path -- phase 3 keeps
    ...
    if _HOMEPAGE_INDEX is None:
        idx: dict[str, dict] = {}
        recon_path = RESEARCH_DIR / "wo323_recon.jsonl"
