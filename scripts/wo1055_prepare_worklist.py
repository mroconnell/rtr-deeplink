"""WO-1055: build the worklist for the 2026-09-24 large-county audit's page
...
4. Writes `reports/wo1055_worklist.csv` in `repair_wrong_pages.py`'s own
   format, and prints one line per row.

Then run the real tool on that file, dry run first:

    python scripts/repair_wrong_pages.py check reports/wo1055_worklist.csv
    python scripts/repair_wrong_pages.py run reports/wo1055_worklist.csv
    python scripts/repair_wrong_pages.py run reports/wo1055_worklist.csv \\
        --apply --allow-deletes
...
`--base-url`, then `ARCHIVE_BASE_URL`, then `http://127.0.0.1:$PORT`.
    ...
    python scripts/wo1055_prepare_worklist.py
"""
...
SOURCE_ENTRY = "WO-1055: 2026-09-24 large-county audit loose ends"
...
HOST_TARGETS: Dict[str, HostTarget] = {
    ...
    # (see tenant_overrides.csv's WO-1055 evidence for what each page says).
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument(
        "--out", default="reports/wo1055_worklist.csv", help="worklist to write"
    )
