...
# --------------------------------------------------------------------------
# WO-174 continuation hand check (WO-191, 2026-09-11): before a video is
# accepted, look at its own title/channel text for signs it belongs to a
# DIFFERENT identifiable body (Kind A) or is real content from what looks
# like this government's own channel but isn't a deliberative meeting
# (Kind B). Neither is caught by `_looks_wrong_government()` above, which
# only flags a different KIND of general-purpose government (city vs.
# county) or a state legislature/court -- not a different SPECIAL-purpose
# body (school district, regional commission, state agency) or an
# on-channel video that just isn't a meeting (ceremony, recruitment ad,
# news clip). WO-191's own by-hand oEmbed audit of one 798-government
# batch found 8 of 80 "found" videos wrong on inspection, every one from
# this exact gap: a school board, a state DOT district, regional/
# multi-township commissions, a county assessor, and two ceremonies/
# recruitment videos (see BACKLOG_DONE.md's WO-191 entry for the real,
# confirmed examples this phrase list is built from).
#
# Deliberately conservative and phrase-based, like WO-133's/WO-146's own
# checks above -- not a general classifier, and it will not catch a
# mismatch that doesn't name itself in the title/channel text (e.g. a
# proper-noun-only regional body whose name doesn't contain any of these
# generic phrases). A miss here is not new risk: WO-191's own by-hand
# oEmbed check remains the backstop this doesn't replace, and every
# ingest this run makes is still available for that same audit. A false
# trigger only costs one extra candidate-video try (Kind B) or one
# skipped government whose real owner gets recorded for a human to check
# (Kind A) -- see `record_hand_check_flag()`.
# --------------------------------------------------------------------------
