"""WO-322 (2026-09-12): passive discovery v2 at full scale, phase 3 -- targeted
...
WO's own redesign calls for:
  ...
  - **Catch-all test.** One nonsense path per host
    (`/__wo322_nonsense_<random>__`), fetched once per government and
...
Output: `research/wo322_targeted.csv`, one row per (government, URL)
...
Usage (from the rtr-deeplink repo root, shared venv):
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo322_targeted_scratch.db" \\
        .venv/bin/python scripts/wo322_targeted.py --limit 50 --concurrency 32
"""
...
CLASSIFIED_CSV = RESEARCH_DIR / "wo322_classified.csv"
TARGETED_CSV = RESEARCH_DIR / "wo322_targeted.csv"
PORTAL_GUARD_CSV = RESEARCH_DIR / "wo322_portal_guard_hosts.csv"
...
# WO-322 change 4: a Wistia/Vimeo/YouTube video reached through a payment,
...
# WO-322: no YouTube calls of any kind, not even a plain page fetch of a
# youtube.com/youtu.be URL -- WO-283 made 439 such fetches (plus 46
# deeper checks) before catching itself mid-run (see BACKLOG_DONE.md's
# WO-283 entry). A candidate URL landing on one of these hosts is never
# fetched here; it is recorded as an unverified lead in the shared
# youtube_channel_leads.csv for the drip process instead.
YOUTUBE_LEADS_CSV = RESEARCH_DIR / "youtube_channel_leads.csv"
YOUTUBE_HOST_SUFFIXES = ("youtube.com", "youtu.be")
_youtube_leads_lock = threading.Lock()
_youtube_leads_seen: set[str] | None = None
_youtube_pinned_hosts: set[str] | None = None


def is_youtube_url(url: str) -> bool:
    netloc = urlparse(url).netloc.lower()
    return any(
        netloc == suf or netloc.endswith("." + suf) for suf in YOUTUBE_HOST_SUFFIXES
    )


def _load_youtube_dedupe_sets() -> tuple[set[str], set[str]]:
    global _youtube_leads_seen, _youtube_pinned_hosts
    if _youtube_leads_seen is not None and _youtube_pinned_hosts is not None:
        return _youtube_leads_seen, _youtube_pinned_hosts
    seen: set[str] = set()
    if YOUTUBE_LEADS_CSV.exists():
        ...
        for row in reader:
            u = (row.get("channel_url") or "").strip()
            if u:
                seen.add(u)
    pinned: set[str] = set()
    pins_csv = (
        ...
        with open(pins_csv, newline="", encoding="utf-8") as f:
            for row in csv.DictReader(f):
                ...
                if host and any(
                    ...
                    for suf in YOUTUBE_HOST_SUFFIXES
                ):
                    pinned.add(f"{host}|{match}")
    _youtube_leads_seen = seen
    _youtube_pinned_hosts = pinned
    return seen, pinned


def record_youtube_lead(
    url: str, gov_id: str, government: str, state: str, kind: str, note: str
) -> bool:
    """Appends an unverified YouTube lead to the shared leads file,
    deduping against the file itself and against existing pins. Returns
    True if a new row was written."""
    seen, pinned = _load_youtube_dedupe_sets()
    with _youtube_leads_lock:
        if url in seen:
            return False
        netloc = urlparse(url).netloc.lower()
        if netloc in {h.split("|")[0] for h in pinned}:
            return False
...
def nonsense_path() -> str:
    ...
    return f"/__wo322_nonsense_{token}__"
...
def load_homepage_cached(domain: str) -> dict:
    ...
    wo322_recon.jsonl for its cached homepage HTML path -- phase 3 keeps
    ...
    if _HOMEPAGE_INDEX is None:
        idx: dict[str, dict] = {}
        recon_path = RESEARCH_DIR / "wo322_recon.jsonl"
...
def process_with_candidates(row: dict) -> list[dict]:
    ...
    for rank, c in enumerate(candidates[:5], start=1):
        if is_youtube_url(c["url"]):
            record_youtube_lead(
                c["url"], gov_id, name, state, "single_video", "wo322-phase3-candidate"
            )
            results.append(
                {
                    "domain": domain,
