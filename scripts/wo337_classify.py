"""WO-337 (2026-09-12): passive discovery v2 at full scale, phase 2 -- offline
classification over phase 1's raw recon file (`wo337_recon.jsonl`).
...
Output: `research/wo337_classified.csv`, one row per government, with a
...
Usage (from the rtr-deeplink repo root, shared venv):
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo337_classify_scratch.db" \\
        .venv/bin/python scripts/wo337_classify.py
...
def classify_record(rec: dict) -> dict:
    ...
    # WO-337 caution 1: for this population, `domain` is sometimes itself a
