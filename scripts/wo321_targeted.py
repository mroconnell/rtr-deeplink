"""WO-321 (2026-09-12): passive discovery v2 at full scale, phase 3 -- targeted
...
WO's own redesign calls for:
  ...
  - **Catch-all test.** One nonsense path per host
    (`/__wo321_nonsense_<random>__`), fetched once per government and
...
Output: `research/wo321_targeted.csv`, one row per (government, URL)
...
Usage (from the rtr-deeplink repo root, shared venv):
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo321_targeted_scratch.db" \\
        .venv/bin/python scripts/wo321_targeted.py --limit 50 --concurrency 32
"""
...
CLASSIFIED_CSV = RESEARCH_DIR / "wo321_classified.csv"
TARGETED_CSV = RESEARCH_DIR / "wo321_targeted.csv"
PORTAL_GUARD_CSV = RESEARCH_DIR / "wo321_portal_guard_hosts.csv"

# WO-321: absolute rule for this run (repeated in the brief because WO-283
...
# WO-321 change 4: a Wistia/Vimeo/YouTube video reached through a payment,
...
def nonsense_path() -> str:
    ...
    return f"/__wo321_nonsense_{token}__"
...
def load_homepage_cached(domain: str) -> dict:
    ...
    wo321_recon.jsonl for its cached homepage HTML path -- phase 3 keeps
    ...
    if _HOMEPAGE_INDEX is None:
        idx: dict[str, dict] = {}
        recon_path = RESEARCH_DIR / "wo321_recon.jsonl"
    ...
    for rank, c in enumerate(candidates[:5], start=1):
        if is_youtube_url(c["url"]):
            record_youtube_lead(
                ...
                name,
                state,
                note=f"WO-321 phase-3 candidate rank={rank} source={c.get('source', '')}",
            )
            results.append(
                {
                    "domain": domain,
    ...
    if html:
        ...
        if best and is_youtube_url(best[0]):
            record_youtube_lead(
                ...
                name,
                state,
                note="WO-321 fallback-rung1-onehop best link",
            )
            best = None
        ...
        if content:
            ...
            if best and is_youtube_url(best[0]):
                record_youtube_lead(
                    ...
                    name,
                    state,
                    note="WO-321 fallback-rung2-headless best link",
                )
                best = None
            if best and third_party_portal_host(best[0], domain):
                record_portal_guard(
