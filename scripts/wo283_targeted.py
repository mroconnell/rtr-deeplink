"""WO-283 (2026-09-12): passive discovery v2 at full scale, phase 3 -- targeted
...
WO's own redesign calls for:
  ...
  - **Catch-all test.** One nonsense path per host
    (`/__wo283_nonsense_<random>__`), fetched once per government and
...
Output: `research/wo283_targeted.csv`, one row per (government, URL)
...
Usage (from the rtr-deeplink repo root, shared venv):
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo283_targeted_scratch.db" \\
        .venv/bin/python scripts/wo283_targeted.py --limit 50 --concurrency 32
"""
...
CLASSIFIED_CSV = RESEARCH_DIR / "wo283_classified.csv"
TARGETED_CSV = RESEARCH_DIR / "wo283_targeted.csv"
PORTAL_GUARD_CSV = RESEARCH_DIR / "wo283_portal_guard_hosts.csv"
...
# WO-283 change 4: a Wistia/Vimeo/YouTube video reached through a payment,
...
def nonsense_path() -> str:
    ...
    return f"/__wo283_nonsense_{token}__"
...
def load_homepage_cached(domain: str) -> dict:
    ...
    wo283_recon.jsonl for its cached homepage HTML path -- phase 3 keeps
    ...
    if _HOMEPAGE_INDEX is None:
        idx: dict[str, dict] = {}
        recon_path = RESEARCH_DIR / "wo283_recon.jsonl"
