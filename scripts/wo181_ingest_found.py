"""WO-181 (2026-09-10) Part A, step 4: resolve/ingest the governments
whose `wo181_report.csv` outcome is "found" -- an alternate domain
answered with a real platform link, per `scripts/wo181_pilot.py`.

Reuses the SAME resolve/ingest pipeline every other sweep in this repo
uses, `wo134_confirmed_hits_ingest.process_row()` -- not reimplemented
here, per CLAUDE.md's "delegate rather than writing a redundant native
parser" convention applied one level up (delegate to the shared sweep
driver, not just the shared platform adapter). Two stages, both run in
one pass since WO-181's own found list is small (a handful of
governments, not wo147's 850-row scale that needed two separate runs):

1. For each found row, build the same `synthetic_row` shape
   wo147_access_ladder_sweep.py builds (gov_id, unit_name, homepage,
   hop2_urls="", hit_source_urls="platform=hit_url") and call
   `wo134.process_row()`. A tier-1/2 result (real captions) is ingested
   directly by that call, POSTing to the Archive's own
   `POST /internal/ingest` -- this script never writes to the database
   itself, it is HTTP-only against the live Archive, per CLAUDE.md's
   "ingest is HTTP-only" rule.
2. A tier-3 result (real video, no reachable captions) is NOT queued
   ...
   here (`wo181_tier3_pending.csv`), the same "probe before queue"
   pattern `wo147_access_ladder_sweep.py`/`wo147_finish_tier3_queue.py`
   established, so every tier-3 candidate is probed
   (`app.platforms.queue_probe.probe_queue_entry()`, WO-144) for a dead
   link or a too-short clip before it ever reaches the real queue file.
   Accepted candidates are appended to the shared
   `scripts/tier3_auto_transcription_queue.txt` and (for a shared host
   like YouTube/Vimeo/TelVue/Cablecast) pinned in
   `app/utils/jurisdiction_data/tenant_overrides.csv` at `fallback`
   strength -- both using wo134's own existing-entry/existing-pin
   dedup helpers, so a re-run never double-queues or double-pins.

Deliberately does NOT import wo147_access_ladder_sweep.py: that module's
own top level runs `wo134.TIER3_HANDLER = tier3_pending_handler`
unconditionally (see coverage_alternates.rungs_2_3_ladder_fn's docstring
for the full explanation), which would silently redirect this script's
tier-3 candidates into wo147's OWN pending file instead of this WO's.
This script sets `wo134.TIER3_HANDLER` itself, after importing
`wo134_confirmed_hits_ingest` directly and nothing else that touches it.

Also writes `wo181_discovery_seeds.csv` (gov_id, tenant, platform,
access_mode) for every found government's answering tenant, per WO-181's
"hand any answering tenant to rtr-discovery as a seed row" instruction --
this script never touches rtr-discovery's own code or ledger, only this
one plain CSV in rtr-business/research.

Usage:
    python scripts/export_meeting_inventory.py --out-dir /tmp/wo181_inv --source export
    DATABASE_URL="sqlite+aiosqlite:////tmp/wo181_ingest.db" \\
        python3 scripts/wo181_ingest_found.py --inventory-csv /tmp/wo181_inv/meeting_inventory.csv
"""
...
INGEST_REPORT_CSV = RESEARCH_DIR / "wo181_ingest_report.csv"
PENDING_CSV = RESEARCH_DIR / "wo181_tier3_pending.csv"
DISCOVERY_SEEDS_CSV = RESEARCH_DIR / "wo181_discovery_seeds.csv"
...
def _wo181_tier3_pending_handler(
    *, gov_id, unit_name, platform, final_seed, hit_url, title, date, result
) -> None:
    """wo134.TIER3_HANDLER for this script -- same shape as
    wo147_access_ladder_sweep.tier3_pending_handler(), writing to THIS
    WO's own pending file instead."""
    ...
    if platform in wo134.SHARED_HOST_PLATFORMS:
        ...
        if host and match:
            pin_row = (
                f"{host}|{match}|{gov_id}|fallback|wo181_ingest_found|"
                f"{unit_name} -- WO-181 alternate-domain find, gov_id={gov_id}"
            )
...
wo134.TIER3_HANDLER = _wo181_tier3_pending_handler
        ...
        for i, row in enumerate(found_rows):
            ...
            try:
                result = await wo134.process_row(
                    session, synthetic_row, covered_gov_ids, "wo181_ingest_found"
                )
