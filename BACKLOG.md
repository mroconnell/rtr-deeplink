...
Open bugs — real, root cause not settled `[NEEDS-AUDIT]`  (213)
  ...
  Adapter & platform gaps  (64)
    ...
    [NEEDS-AUDIT] The passive pipeline's "nothing…
    [NEEDS-AUDIT] `passive_verify.py`'s `video_found` doesn't check…
...
- **[NEEDS-AUDIT] The passive pipeline's "nothing walkable"/"candidate-not-confirmed" verdicts have a measured ~62% real false-negative rate — most of these governments DO have a findable agendas/minutes page; the single-hop, homepage-only scan just isn't reaching it.**
  - **Issue**: WO-347's 40-government hand audit (20 from the "nothing walkable" bucket, 20 from WO-337's `candidate-not-confirmed` outcome) found 25 real, live agendas/minutes pages by hand that the automated sweep had recorded as "no meeting found at all" — real dated PDF archives, real CivicPlus/CivicWeb/CivicClerk/IQM2 listings, and clearly-named first-party pages (`/meeting-agendas.html`, `/zoning-minutes/`, a 300+-link council archive) sitting one or two clicks past what the sweep's own homepage-only hop scan reached. One (Eatwp township, PA) had a full platform (a real CivicWeb tenant) the sweep never found at all, buried several clicks into a static board page. By contrast, the SAME audit's tier-4 ("meeting, no video") stratum was 100% correct on 20/20 — the sweep is reliable once a platform is confirmed; the failure is upstream, in finding the platform/page at all.
  - **Impact**: at a ~62% false-negative rate, the "nothing walkable" (146 governments as of WO-338's rerun) and `candidate-not-confirmed` (1,145 governments, WO-337's own largest bucket) buckets very likely contain hundreds of governments with a real, resolvable meetings page recorded as having none — a materially undercounted coverage picture, not just a handful of misses.
  - **Next action**: extend the hop search past a single homepage hop for these two buckets specifically — either a second hop from the first hop's own best-scored sub-links, or a short list of guessable deeper paths (`/zoning/minutes`, `/boards-commissions/*/minutes`, a site's own internal search) before giving up. `verify_hub()`'s existing `first_party_agenda_page` guessable-path fallback (added WO-333) already proves the pattern works; it just needs a wider path list and a real second hop, not just the bare homepage.
  - **Constraint**: don't rebuild this against the full 1,291-government population before re-testing the fix against a fresh, independent sample — 40 governments is enough to establish the rate is real and large, not enough to certify a specific fix's yield.
  - **History**: `BACKLOG_DONE.md`, WO-347, 2026-09-13 (`research/wo347_audit_sample.csv`, `research/wo347_report.csv`).

- **[NEEDS-AUDIT] `passive_verify.py`'s `video_found` doesn't check `Content-Type` before trusting a populated `video_url` — a CivicClerk `video_url` can be a genuine audio-only file, not video.**
  - **Issue**: confirmed live 2026-09-13 (WO-347): Olmos Park city, TX's CivicClerk listing walker returned a real, live `video_url` ending in `.mp3`; a direct `HEAD` request confirms `Content-Type: audio/mp3` (a real 58 MB file). This is the exact failure shape `civicclerk.py`'s own module docstring already documents for Highland, CA (`externalVideoUrl` pointing to an MP3, not video) — but nothing in `verify_hub()` or the adapter's own resolve path checks the media type before calling it a real video.
  - **Impact**: at least one real government (Olmos Park, TX) is one `Content-Type` check away from being wrongly queued to tier 3 as "video, no captions" when it has no video at all, only audio — a violation of Ryan's "only meetings with video become pages" rule if it goes uncaught. Scope beyond this one sample is unmeasured.
  - **Next action**: add a cheap `HEAD` request (or reuse `media_probe`'s existing duration-probe machinery, which already fetches headers) checking `Content-Type` starts with `video/` (or a known HLS/manifest shape) before `verify_hub()`/`queue_probe.py` treat a populated `video_url` as real video; an audio `Content-Type` should record the same "meeting, no video" verdict CivicClerk's own audio-only case already gets elsewhere.
  - **Constraint**: one confirmed live example (Olmos Park, TX) — worth a quick grep of already-queued tier-3 URLs for other `.mp3`/`audio` shapes before assuming this is isolated.
  - **History**: `BACKLOG_DONE.md`, WO-347, 2026-09-13.
