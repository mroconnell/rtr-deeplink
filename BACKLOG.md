...
## [JUST-DO-IT] Granicus's CloudFront blocks a residential IP after a handful of rapid stream requests -- confirmed live, explains a run of "ffmpeg timed out" false negatives (2026-08-20)

**Confirmed directly, not inferred from the timeout alone.** During a
session testing `vad_filter=True` (#216) against real garbled-transcript
examples, three re-transcription attempts each against two different
long Granicus meetings (Napa CA clip 5812, 18 chunks; King County WA clip
11547, 8 chunks) all failed identically: `ffmpeg` hung for exactly 120s
extracting chunk 1 at offset 0s, then hit `media_probe.py`'s own
subprocess timeout (see BACKLOG.md's existing "no retry" entry on this
same script for a different, unrelated cause of a similar-looking
skip). A direct `curl` against the exact same
`archive-stream.granicus.com` playlist URL immediately (85ms) returned
**403, CloudFront "Request blocked"** -- real, confirmed, not a guess:
...
A shorter meeting (Alexandria VA clip 6603, 2 chunks) succeeded cleanly on
its *second* attempt earlier in the same session, before this block
kicked in -- consistent with the block being a rate/volume trigger (this
session made several rapid Granicus requests across three different
jurisdictions' streams, plus `/api/resolve` calls, within about 20
minutes), not a per-URL or per-meeting-length problem. `ffmpeg`'s 120s
hang rather than a fast 403 is presumably it retrying/stalling against
CloudFront's block internally rather than surfacing the HTTP status the
way a plain `curl` does -- not yet confirmed by reading `ffmpeg`'s own
verbose output, so treat that specific mechanism as a reasonable
explanation, not a verified one.

**Not yet fixed.** Whether this is a short-lived per-request-burst
throttle (clears on its own after a cooldown) or a longer IP-level block
is unconfirmed -- didn't want to keep hammering it mid-session to find
out. Real implications for `scripts/transcribe_backlog_locally.py`'s
batch mode: a real overnight run works through many Granicus meetings in
a row, and if this is genuinely volume-triggered, a full-backlog run
could trip the same block partway through and silently degrade into a
string of "ffmpeg timed out" skips that look like random flakiness (the
existing retry-on-next-run pattern from BACKLOG.md's other ffmpeg-timeout
entry) rather than the real cause. Worth: (1) confirming whether/when the
block clears, (2) adding a real backoff/pacing between Granicus requests
in the backlog script if it's confirmed volume-triggered, (3) having
`process_one()`/`extract_chunk_audio()` detect a CloudFront-block-shaped
