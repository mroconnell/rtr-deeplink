...
Open bugs — real, root cause not settled `[NEEDS-AUDIT]`  (226)
...
- **[HUMAN] Run the re-transcription queue for the pre-voice-filter Whisper pool: pilot, review, promote, then the other 77 pages (WO-929, from WO-928).**
  - **Issue**: 82 pages show only older Whisper text (made before the voice filter, or a repair copy of it) with a real defect in the text: 46 with the silence signature ("Thank you." every 30 seconds), 35 with loops, 1 other. None carries a warning marker, so the cloud worker never picks them up. Ryan decided (2026-09-21) to re-transcribe them on the local Whisper machine through their own queue, `scripts/retranscription_queue.txt` (5-page PILOT, 77-page MAIN, an empty DRIP-MAC-ONLY section; sidecar `scripts/retranscription_queue_meta.csv`; runbook `docs/RETRANSCRIPTION_QUEUE.md`). All 82 re-checked live on 2026-09-21: shown version unchanged, defect still present, video resolves.
  - **Impact**: 254.3 hours of meeting audio (pilot 9.1 h, main 245.2 h). The local machine's speed is not measured yet, so the run time is unknown; the runbook's step 1 measures it.
