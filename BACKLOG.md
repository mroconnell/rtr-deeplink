...
Open bugs — real, root cause not settled `[NEEDS-AUDIT]`  (176)
  ...
  [NEEDS-AUDIT] Two real Kind-A finds from WO-264's hand-read gate,…
...
- **[NEEDS-AUDIT] A real ProudCity or viebit tenant page named "watch live"/"video archive"/a folder listing can exist and still make the adapter extract nothing at all — two confirmed real examples, cause not yet read.**
  ...
  - **History**: `docs/investigations/passive_discovery_phase3b.md`'s "Hand-read gate" section (WO-281, 2026-09-12).
- **[NEEDS-AUDIT] `[BIG]` WO-281's homepage-hop sweep confirmed a real platform on 249 more governments (98 bare YouTube channel links, 152 confirmed only via a different link on the same fetched page) that still need a per-government hand-check before any of them can become a page or a queue line.**
  - **Issue**: `research/wo281_targeted.csv` (`platform_confirmed != ""`) has 249 governments beyond the 13 WO-281 itself hand-checked and applied. 98 are a bare YouTube channel URL (needs its own video found and the channel identity confirmed, per this repo's Kind-A/Kind-B rule) and 152 were confirmed only because SOME link on the fetched page carried a platform signal, not the specific scored candidate link itself (needs the actual page re-read to find which link is the real one).
  - **Impact**: a real, sizeable population of governments (roughly 3x WO-273's own comparable yield on this same population, see the investigation doc) sitting undiscovered until someone does the per-government read.
  - **Next action**: the 98 YouTube channels are `scripts/youtube_drip.py`'s job (its own Mac, per the 2026-09-11 ownership decision) — hand them to that pipeline rather than a fresh bulk channel-enumeration run. The 152 "weak" governments need a plain per-government hand-read pass over their own `wo281_scored.csv` top-5 rows.
  - **Constraint**: don't bulk-enumerate the 98 YouTube channels from a session other than the drip's own Mac — CLAUDE.md's YouTube 429-block rule and the drip-ownership note both apply.
  - **History**: `docs/investigations/passive_discovery_phase3b.md` (WO-281, 2026-09-12).
...
- **[NEEDS-AUDIT] Two real Kind-A finds from WO-264's hand-read gate, neither government's own row corrected yet: East China Township, MI and Chisholm city, MN.**
  - **Issue**: WO-264's hand-read gate (2026-09-12) caught two candidates that were real meetings of a DIFFERENT government than the one the sweep recorded them against. Marine City city, MI: the candidate video is "East China Township Parks Commission Mgt., Sept. 9, 2026" on a shared regional channel ("CTV Community Television") — East China Township's own meeting, not Marine City's. Mountain Iron city, MN: the candidate video is "Chisholm City Council, September 9th, 2026" on a shared regional channel ("Mesabi Community TV") — Chisholm's own meeting, not Mountain Iron's. Neither was ingested against the wrong government (the hand-read gate caught both before any page was created).
  - **Impact**: none live-facing (nothing was ingested wrong) — both are real leads sitting unused: East China Township and Chisholm each have a real, on-mission meeting recording that could become a page under their own `gov_id`, once looked up.
  - **Next action**: look up East China Township, MI's and Chisholm city, MN's own `gov_id`s and resolve/ingest each video against the correct government instead.
  - **Constraint**: don't pin either video to Marine City or Mountain Iron under any circumstance — neither owns this content.
  - **History**: `BACKLOG_DONE.md`'s WO-264 entry; `rtr-business/research/ENUMERATION_METHODS.md` §303.
