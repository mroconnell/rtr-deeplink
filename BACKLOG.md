...
Open bugs — real, root cause not settled `[NEEDS-AUDIT]`  (216)
  ...
  Adapter & platform gaps  (64)
    ...
    [NEEDS-AUDIT] Even after WO-348's "look one hop deeper" fix, a real…
...
- **[NEEDS-AUDIT] Even after WO-348's "look one hop deeper" fix, a real tier 1/3 "video found" verdict is still wrong most of the time on hand-read — the CivicClerk listing walker in particular picks the newest event with media without checking it's a real governing-body meeting.**
  - **Issue**: WO-348's own group-1 rerun (1,514 governments) produced only 11 tier 1/3 (video-found) candidates; hand-reading all 11 found 8 wrong: Forest Park city, GA's CivicClerk event 1411 is literally titled "TEST 3" (empty agenda, `agendaName: null`) — the walker's own "newest event with media" rule has no check that the event is a real meeting, not a test upload; Winter Garden city, FL's CivicClerk event externalVideoUrl 404s (a real "City Commission Meeting" event, but the linked video file is dead); Boley town, OK's candidate is an event-registration page for a rodeo, not a meeting; Dummerston town, VT's candidate is a homepage decorative background video (`img/videobg.mp4`); Coatesville town, IN and Genoa village, WI's recorded domains have both been fully repurposed by an unrelated (apparently Indonesian gambling-branded) site, and both "found" the exact same generic Shopify stock video file — a shared-template false positive, not two real government videos; Pea Ridge city, AR's candidate is a "State of the City" mayoral address video, not a governing-body meeting recording (borderline judgment call, flagged for Ryan). Only Libertyville village, IL (Vimeo, confirmed via oEmbed: "Village of Libertyville", real dated meeting) and Enumclaw city, WA (CivicMedia, confirmed real "Regular Meeting") were real and ingested; Scandia city, MN (BoxCast) was already hand-verified and queued.
  - **Impact**: the hop-deeper fix materially helps tier 4 ("meeting, no video") verdicts, but a tier 1/3 "video found" verdict specifically still needs a human every time — at this sample size (11), roughly 3 in 4 were wrong. A future rerun that skipped the hand-read gate on tier 1/3 would have ingested or queued at least 6 bad pages/queue lines from this one 1,514-row chunk alone.
  - **Next action**: for the CivicClerk walker specifically, reject an event whose `eventName` is a generic test-looking string (regex like `^TEST\b`, case-insensitive) or whose `agendaId`/`agendaName` is null/0, and prefer the next-newest real event instead; separately, HEAD-check `externalVideoUrl` before crediting it as reachable video (same pattern as the audio-only-file fix WO-348 already added for content-type). For the generic `direct_file`/homepage-video-tag path, add a floor: reject a candidate video whose filename matches a known decorative pattern (`videobg`, `background`, `hero`) or whose exact same URL/hash already matched a different government in the same run (the Coatesville/Genoa Shopify collision) — a repeated hash across unrelated domains is itself strong evidence of a shared-template false positive.
  - **Constraint**: 11 candidates is a real but small sample — don't tune thresholds tightly to these 8 wrong ones alone; get another sample before calling any fix proven.
  - **History**: `BACKLOG_DONE.md`, WO-348 (resume), 2026-09-13.
