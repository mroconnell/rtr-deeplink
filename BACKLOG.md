...
Ship next — root cause known, fix settled `[JUST-DO-IT]`  (26)
  ...
  Coverage registry: per-state view and other dashboard additions…  (7)
...
Open bugs — real, root cause not settled `[NEEDS-AUDIT]`  (136)
  ...
  Adapter & platform gaps  (44)
    ...
    [NEEDS-AUDIT] Legistar/CivicPlus's delegation-to-YouTube "known…
...
- **[NEEDS-AUDIT] Legistar/CivicPlus's delegation-to-YouTube "known quirk" now also blocks that page's government identity, since WO-210's multi-gov-host safeguard can't tell a trustworthy delegated name from an untrustworthy channel guess.**
  - **Issue**: `civicplus.py`/`legistar.py` delegate a linked YouTube video to `resolve_via_platform()`, which returns the YouTube result AS-IS — only `.jurisdiction` is overwritten (from the delegating page's own subdomain/agenda text), never `.source_url` (CLAUDE.md's own "known quirk" bullet). So `_resolve_page_government()` computes `host` from the FINAL, delegated `source_url` (`youtube.com`), not the originating government's own host. WO-210's new rung 1b in `resolver._resolve_government_ladder()` returns no government for ANY multi-gov host absent a matching per-video/channel pin — and it cannot distinguish "this name came from CivicPlus's own trusted subdomain" from "this name is an untrusted YouTube channel guess," because both arrive as the same plain `raw_name` string once delegation has already happened.
  - **Impact**: a Legistar/CivicPlus page that delegates to an UNPINNED YouTube/Vimeo video, whose delegating page's own jurisdiction extraction would have resolved correctly on its own, now resolves to no government at all instead — a real, deliberate widening of WO-210's safeguard (see `docs/COVERAGE_HANDOVER.md`'s identity section), not a bug in the safeguard itself. Scale unmeasured: nobody has swept how many already-archived pages took this exact path (delegated `source_url`, no `tenant_overrides.csv` row, previously-successful national-table resolution) as of this writing.
  - **Next action**: either (1) fix the underlying quirk — keep `source_url` on the DELEGATING host, the way `PrimeGovAssetFinder` already does (calling `YouTubeAssetFinder.resolve_video_id()` directly rather than `resolve_via_platform()`), which would let CivicPlus/Legistar's own subdomain-derived jurisdiction keep resolving without a pin AND stay inside the new safeguard, or (2) run a real sweep (Archive export, filter to `platform IN (legistar, civicplus)` pages whose `source_url` host is in `MULTI_GOV_HOSTS`) to size how many pages this actually affects before deciding it's worth (1)'s adapter change.
  - **Constraint**: don't "fix" this by loosening `resolve_government()`'s new rung 1b itself — that is Ryan's explicit rule (`docs/COVERAGE_HANDOVER.md`), and weakening it to accommodate delegation would reopen the exact risk it closes (an untrusted channel/video guess resolving with no pin).
  - **History**: found while building WO-210, 2026-09-11 — see that WO's PR and `app/utils/gov_registry/resolver.py`'s own rung-1b comment for the full reasoning.
