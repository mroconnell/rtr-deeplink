...
Ship next — root cause known, fix settled `[JUST-DO-IT]`  (7)
  ...
  Coverage registry: per-state view and other dashboard additions…  (6)
    ...
    [JUST-DO-IT] A queued video's duration/date/size can be probed…
...
- **[JUST-DO-IT] A queued video's duration/date/size can be probed cheaply without downloading it — confirmed live across every tier-3 platform, with a concrete per-platform recipe and a real dead-link rate.**
  - **Issue**: `feed_tier3_auto_transcription.py`'s `_push_if_has_video()` pushes any queue line straight to `/internal/ingest` the moment `finder.resolve()` returns a `video_url` — no duration/size/plausibility check runs before the page becomes a real Archive `MeetingPage`. The only duration gate in the whole pipeline is `worker/main.py`'s `probe_duration()` + `is_plausible_meeting_duration()` (60s-14h), which runs at *claim* time, after ingest. A live 25-entry probe (WO-143, 2026-09-10; 20 entries added to the queue 2026-09-09/10 plus 5 older, spanning YouTube/Granicus/CivicClerk/Vimeo/Swagit/Cablecast/TelVue) confirmed every one of those platforms can be probed for duration (and, on some, size) without downloading media: YouTube via yt-dlp `extract_info(download=False)` (reusing `youtube.py`'s android/ios/tv/web `player_client` order, ~1.6-2.7s, zero media bytes); a `.m3u8` URL (Granicus, Swagit, and Cablecast all resolve through Granicus's `archive-stream.granicus.com` CDN or a same-shaped Cablecast HLS host) via 2 small GETs — the master playlist alone reports 0 segments, the variant it names carries the real `#EXTINF` sum (127 bytes + 4-442KB, 0.3-2s); a direct `.mp4` (CivicClerk) via one HEAD for `Content-Length`/`Last-Modified` plus the existing `media_probe.probe_duration()` (already used by the worker, 0.7-3.1s); Vimeo via the adapter's own oEmbed call (`vimeo.com/api/oembed.json`, ~0.2-0.3s, 1.2KB), which already returns `duration`/`upload_date` today but doesn't store them on `ResolvedMeeting`; TelVue via the page's own embedded `Player.setupData` JSON, which already carries `duration` for free (same page `resolve()` already fetches) — **except both TelVue samples probed had `duration: 0` because the resolved stream was a live placeholder (`livestream.telvue.com`), not an archived VOD, so a raw `duration == 0` read off that JSON is not trustworthy on its own.** 6 of the 25 (24%) were dead by other signals: 2 YouTube videos were scheduled-but-not-yet-live ("This live event will begin in..."), 1 CivicClerk resolve timed out at 20s, 1 Cablecast resolve returned no `video_url` at all, and the 2 TelVue live-placeholders above. One alive entry (Aledo ISD, TX, Swagit, 35.7s) was also below the existing 60s `MIN_PLAUSIBLE_MEETING_SECONDS` floor — a real short/ad-shaped clip, not a meeting.
  - **Impact**: dead links and implausible-length clips reach the Archive as real pages today, not just as wasted transcription jobs — worker-side duration gating only protects a transcription job's cost, it does nothing about the page itself, and it doesn't run until whenever the worker happens to claim that job (which can be hours/days after ingest). Concretely in this sample: Aledo ISD's 35.7s clip and Anaheim's real-but-8.45h Granicus meeting (30,411.5s — over a naive 6-hour ceiling despite being a genuine government recording) both illustrate that duration alone, applied as a hard ceiling, isn't safe without care.
  - **Next action**: add a small `probe_queue_entry()` helper (module home should sit beside `media_probe.py` so both feed scripts and the worker can share it) implementing the per-platform recipe above, called from `_push_if_has_video()` right after `finder.resolve()` succeeds and before `_ingest()` — same wiring point in `feed_granicus_auto_transcription.py`. Record url/platform/duration/date/size/probe_seconds/verdict as an append-only `tier3_auto_transcription_queue_probe.csv` sidecar next to the queue file (the queue itself stays a plain URL list, unchanged — same resumable-log shape already asked for two entries up in this section), and skip the push (`[SKIP] implausible duration ...`) on REJECT rather than only on "no video found on re-resolve." Deliberately at **queue-feed time, not claim time**: the worker's existing claim-time gate stays as a second, independent check (a resolve can drift between queue-feed and claim), but only a pre-ingest check stops an implausible/dead page from becoming a real Archive page at all.
  - **Constraint**: treat duration as the primary signal wherever it's cheaply available (every platform probed here) — a size floor is a fallback for when duration genuinely can't be had, not a parallel check; e.g. HLS has no cheap *total* size (no `Content-Length` on a playlist), only a per-segment sample requiring one extra HEAD, so size-based rejection isn't worth building for Granicus/Swagit/Cablecast specifically. Observed bytes/minute this session (for sizing that fallback if it's ever built): CivicClerk direct mp4 ~15.3-39.1 MB/min (3 samples, avg ~28 MB/min); Granicus/Swagit HLS ~14.7 MB/min (one 720p segment sample); Cablecast HLS ~24.1 MB/min (one 1080p-rendition segment sample — the master playlist's own declared `BANDWIDTH` per rendition is a free, cheaper proxy for the others); YouTube ~1.6 MB/min (only the 360p progressive format the android/ios/tv clients return with a populated `filesize`, one sample — not representative of what a viewer actually streams); this app's own audio-extraction target (`extract_full_audio()`'s 32kbps mono mp3) is 0.24 MB/min, the only real number available for "the appropriate file size for audio" since no platform here serves an audio-only source. **Don't set a hard multi-hour ceiling** — Anaheim's real 8.45h meeting in this sample would be wrongly rejected by the 6h candidate ceiling; a "very long" duration should log/flag, not hard-skip, same as the existing 14h `MAX_PLAUSIBLE_MEETING_SECONDS` already treats it. Don't reuse this probe as a reason to lower `MIN_PLAUSIBLE_MEETING_SECONDS` below 60s — see the Standing-decisions entry against that.
  - **History**: WO-143, 2026-09-10, this session's live 25-entry probe pilot (Ryan's ask). Full 25-row table, exact per-entry probe times/bytes, and the HLS master/variant playlist findings are in `BACKLOG_DONE.md`.
