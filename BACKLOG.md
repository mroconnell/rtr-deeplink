...
Ship next — root cause known, fix settled `[JUST-DO-IT]`  (54)
  ...
  Coverage registry: per-state view and other dashboard additions…  (13)
    [JUST-DO-IT] `[BIG]` BoardDocs video adapter — mechanism confirmed…
...
- **[JUST-DO-IT] `[BIG]` BoardDocs video adapter — mechanism confirmed live and two real tenants with video in hand (Tallahassee FL, Colorado City AZ schools).**
  - **Issue**: BoardDocs was excluded on 2026-08-29 (§43, "no reliable video"). Tallahassee's City Commission runs on it with per-meeting YouTube video, exposed by a fixed unauthenticated mechanism: tenant flag `bd.videoservice` on the Public page, meeting list from `BD-GETMeetingsListForSEO`, and `POST .../VIDEO-GetAgenda` with `id=<Unique>` returning the YouTube/Vimeo id. Confirmed 2026-09-14 on 10 tenants.
  - **Impact**: Tallahassee (205,000) and any other BoardDocs government with the service on and ids filled in; 4 of 5 flagged tenants carry the flag but no ids, so the per-meeting POST is the only real signal.
  - **Next action**: build `app/platforms/boarddocs.py` per `docs/investigations/boarddocs_video_adapter.md` — flag, list, per-meeting POST, delegate the id to `YouTubeAssetFinder`/Vimeo, keep the `goto?open&id=` page as `source_url`, agenda outline from the same fragment. Fixtures from Tallahassee and Colorado City; Austin ISD, Northside ISD, St. Charles MO, Vacaville USD as negative controls (Ryan's named test set; all returned no ids on their 40 newest meetings).
  - **Constraint**: `go.boarddocs.com/robots.txt` disallows all agents with `Crawl-delay: 1000`; the adapter reads one tenant on demand with a Referer header and never enumerates tenants. `videoservice=3` is unseen live; log and return video-less.
  - **History**: rtr-business `ENUMERATION_METHODS.md` §43 (2026-08-29) and "Follow-up to §43" (2026-09-13); spec 2026-09-14.
