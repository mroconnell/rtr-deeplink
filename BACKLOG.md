...
Open bugs — real, root cause not settled `[NEEDS-AUDIT]`  (199)
  ...
  [NEEDS-AUDIT] Confirmed, quantified, and generalized (WO-331): most…
...
- **[NEEDS-AUDIT] Confirmed, quantified, and generalized (WO-331): most adapters return "no video" on their own real listing/hub page instead of walking to the newest meeting, and it is the dominant cause of "meeting-without-video"/"no-meeting-nor-video" across the whole passive-v2 wave, not a handful of edge cases.**
  - **Issue**: WO-331 (2026-09-13) built a control set of 26 governments KNOWN to have real video (one per platform, sourced from the Archive's own already-ingested pages), ran each through the exact passive-v2 pipeline (phases 1-3 plus the sweeps' own `resolve()` hand-check), then took one extra hop the sweeps never take. Result: of 15 governments where phase 3 confirmed a platform, exactly 1 (Plymouth MI, civiclive-vimeo) got real video straight from the sweep's own hand-check. Taking one further hop found real video for exactly 1 more (Queen Anne's County MD, once the confirmed CivicPlus `/AgendaCenter` page's own embedded Telvue player URL was followed) -- 2 of 15 (13%) in total, across hand-check plus one extra hop combined. The other 13 split three ways, all confirmed live: (1) the confirmed URL IS the platform's own real hub/listing page and `resolve()` returns empty or `CalendarPageError` on it -- Ann Arbor MI's Legistar `Calendar.aspx`, Forest Lake MN's CivicPlus `/AgendaCenter`, Troy NH's own `townhallstreams.com/towns/troy_nh` page (this last one *is* the real vendor hub, handed directly to `resolve()` -- still empty, and townhallstreams.py has no further link to hop to at all); (2) the confirmed URL isn't even the video platform's own URL at all -- it's a same-domain government page that merely links to or names the real platform (Knoxville TN, Niagara Falls ON, Victoria BC, Prince William County VA, Crest Hill IL, Maury County TN all hit `detect_platform() -> unknown` on the "confirmed" URL) -- following the one real vendor link found on that page reached iqm2/civicweb/escribe/granicus for 5 of 6, but every one of those five landed on THAT vendor's own tenant home/listing page in turn (a second, unhandled hop) and still returned no video -- only Queen Anne's County's telvue link happened to be a specific player embed rather than another listing; Crest Hill's `municodemeetings.com` hub was the same shape, `CalendarPageError` one level deeper, confirming `municode_meetings.py` has no listing-walk at all; (3) CivicPlus's own listing walk (the "5 most recent listings" logic this section's previous entries already describe) ran and found nothing, because the real video lives on a *different* embedded platform (iqm2) that walk doesn't follow -- Franklin NH, Monroe County FL, Webb County TX. A parallel hand-check the same day (18 real `no-meeting-nor-video` rows) found 16 of 18 were the same kind of false negative. Cross-referencing `jurisdiction_coverage.csv`: 957 rows carry `no-meeting-nor-video`/`meeting-without-video` with a suspected provider on one of these platforms -- CivicPlus alone accounts for 576 of them (409 meeting-without-video + 167 no-meeting-nor-video), civicweb 125, escribe 61, civicclerk 66, granicus 47, iqm2 38, municode_meetings 22, primegov 16, legistar 3, townhallstreams 3. The wave's own report files (`wo321_final_classification.csv`, `wo324_report.csv`, `wo325_final_classification.csv`, `wo326_report.csv`) independently show the same shape: 176 meeting-without-video rows, civicplus 78 of them (wo320/322/323 haven't reached their own hand-check step yet as of this writing).
  - **Impact**: this is very likely the single largest source of undercounted video across the whole 2026-09 passive-discovery wave -- far bigger than any individual adapter bug filed elsewhere in this section. A government correctly reached, correctly identified, with a real platform link found, is still recorded as having no video 87% of the time in this control set, purely because verification stops at one `resolve()` call on whatever page phase 3 happened to confirm, and a single extra hop mostly just reaches the *next* listing page up rather than a specific meeting.
  - **Next action**: two separable fixes, neither of which a single generic hop (what this WO's own step 5 tried) reliably solves -- a real walk needs to reach a SPECIFIC meeting, not just the right vendor domain. (a) **Verification/adapter fix (affects every future sweep)**: teach each adapter a real listing-walk to its OWN newest specific meeting -- for CivicWeb/escribe/iqm2/granicus/legistar/townhallstreams/municode_meetings, this is the same shape `civicplus.py`'s `NoVideoCandidateFound` path already has for CivicPlus, just missing on the other adapters (`municode_meetings.py` and `townhallstreams.py` confirmed to have NONE at all). (b) **Classifier-ranking fix**: on CivicPlus governments whose real video lives on a different embedded platform (iqm2/telvue/granicus/primegov), phase 2's candidate scoring keeps ranking the CivicPlus `/AgendaCenter` link above the real video platform's own link, so phase 3 "confirms" the wrong (but real) platform -- Franklin NH/Monroe County FL/Webb County TX all show this. Given CivicPlus alone is 576 of 957 affected research-file rows, it is the one worth fixing first if only one gets picked. **Do not bulk-rerun the wave until at least the CivicPlus and CivicWeb/escribe/iqm2 walks exist** -- rerunning now would just repeat the same false negatives at the wave's normal cost.
  - **Constraint**: one control government per platform -- real, live-verified, but not yet a multi-tenant sample per platform (this repo's own "test against a real URL first" rule is satisfied per-platform, not yet at the "several tenants" bar `civicplus.py`'s own entries above already cleared for CivicPlus specifically). The one-hop follow-up used a one-off generic link-picker (documented as such in `scripts/wo331_vendor_hop.py`, not the production hop scorer), so its negative results show that a NAIVE single hop doesn't fix this, not that a well-built adapter-specific walk couldn't. WO-320/322/323 have not reached their own hand-check step yet, so their share of the wave's meeting-without-video count by platform is still unknown.
  - **History**: `BACKLOG_DONE.md`'s WO-331, WO-292, WO-283, WO-281, WO-282 entries; `docs/investigations/passive_discovery_v2.md`; `rtr-business/research/wo331_report.csv`/`wo331_handcheck.csv`/`wo331_vendor_hop.csv` for the full 26-government control set and every `resolve()` call's verdict; `rtr-business/research/ENUMERATION_METHODS.md` §337 (WO-331).
...
- **Issue**: found live 2026-09-12 (WO-282), building a population of
  ...
  WO-283 (2026-09-12) found a related but distinct pattern building its
  own 7,746-row population: 78 `domain` values are shared by 2+ rows,
  and the largest ones are a generic hosting-platform homepage used as a
  placeholder `domain` for many unrelated small governments at once --
  `www.facebook.com` (44 rows), `sites.google.com` (11), `facebook.com`
  (4), `tshaonline.org` (4), `municipalites-du-quebec.com` (3). None of
  these is a real per-government website; fetching "the domain" for any
  one of these rows fetches a shared platform's own generic homepage,
  not that government's content.
- **Impact**: unknown scope beyond these 5 -- found by coincidence while
  ...
  rows (and any others like them). The Facebook/Google Sites placeholder
  pattern is a second, likely larger-scope version of the same root
  problem: "is this really THIS government's own domain" would catch
  both.
