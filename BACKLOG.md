...
- **[NEEDS-AUDIT] PR #807's squashed "move the shadowed-county resolver bug to done" commit deleted this section's entire prior content from `BACKLOG.md` (89 entries) instead of moving just its own one fixed entry — most of those entries are still genuinely open and now sit only in `BACKLOG_DONE.md`, unfixed.**
  - **Issue**: `git show 7b0a171 -- BACKLOG.md` shows the whole `## Open bugs` section (1,369 lines) removed from `BACKLOG.md` in one diff and the same content appended verbatim to `BACKLOG_DONE.md` (each moved entry still reads `[NEEDS-AUDIT]`/"Not yet in `BACKLOG_DONE.md`" — nobody actually fixed them, they were just relocated). Confirmed on 4 spot-checked entries beyond this session's own (the minted-`rtr:`-id state-code false positive, the tier3 queue's collapsed feasibility, a `tenant_overrides.csv` pin gap, Phase 2d's signal-based recovery): all 4 exist only in `BACKLOG_DONE.md`, not in the live `BACKLOG.md`, as of 2026-09-10. A fresh `## Open bugs` section was later recreated in `BACKLOG.md` (by a subsequent PR) with new entries only — it does not include the swept-up ones.
  - **Impact**: real, currently-open work across the whole project is invisible to anyone reading `BACKLOG.md` (which this file's own header says to read *instead of* `BACKLOG_DONE.md` when deciding what to build) — up to ~88 entries beyond this session's one restored entry (see the jurisdiction-misattribution entry above, restored 2026-09-10 after noticing this).
  - **Next action**: someone needs to walk the pre-move `## Open bugs` block (`git show 7b0a171^:BACKLOG.md`, or diff `7b0a171^` against `7b0a171` for the exact removed text) entry-by-entry, decide which are genuinely still open vs. actually resolved since, and restore the still-open ones to `BACKLOG.md` — merging carefully with whatever the freshly-recreated `## Open bugs` section already has, not just re-pasting the old block wholesale (some overlap is likely). Not attempted here beyond this session's own one entry — the full reconciliation is a bigger, more careful task than this session's scope, and touches other sessions' work this session doesn't have context on.
  - **Constraint**: don't just re-paste the removed block back verbatim — some of those 89 entries may have been fixed for real since 2026-09-09 by work this session hasn't reviewed; each needs a quick "is this still true" check (this file's own standing rule on backlog entries decaying) before restoring, not a blind bulk revert.
  - **History**: found 2026-09-10 while updating this session's own jurisdiction-misattribution entry and discovering it had vanished from `BACKLOG.md`. Not yet in `BACKLOG_DONE.md`.
