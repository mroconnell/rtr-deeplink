...
Open bugs — real, root cause not settled `[NEEDS-AUDIT]`  (218)
  ...
  [NEEDS-AUDIT] `/m/{slug}` 500s in production for one real,…
...
- **[NEEDS-AUDIT] `[EASY]` A `vimeo.com` pin shaped `vimeo:<id>` in `tenant_overrides.csv` never actually matches any real Vimeo URL — 8 committed rows are silently dead.**
  ...
  - **Impact**: up to 8 real governments' Vimeo pins are dead weight — the intended government never actually gets keyed by them, and nothing fails loudly to say so (CLAUDE.md's own "flag risky optional config" pattern: this is exactly a silent-on-bad-input case). CLAUDE.md itself documents the `vimeo:<id>` shape as the convention ("Wistia `external_id=`... Vimeo `vimeo:<id>`"), so a future pin is likely to be written the same broken way unless this is fixed.
  - **Next action**: either (a) strip the `vimeo:` prefix at load time in `_load_tenant_overrides()` before the row is stored (cheapest, fixes all 8 existing rows with no data migration), or (b) rewrite the 8 rows to the bare-id shape and update CLAUDE.md's own stated convention to match. Re-verify each of the 8 governments' Vimeo video still resolves to the same id before changing anything (a video can be deleted/replaced).
  - **Constraint**: don't silently change CLAUDE.md's documented convention without also fixing the 8 rows (or vice versa) — the two need to agree, and a test (`tests/test_gov_registry.py` already covers the blank-match safeguard) should assert the `vimeo:<id>` shape actually matches a real sample URL, so this can't regress silently again.
  - **History**: `BACKLOG_DONE.md`'s WO-356 entry; `rtr-business/research/wo356_methods_section.md` (ENUMERATION_METHODS.md §359).

- **[NEEDS-AUDIT] `/m/{slug}` 500s in production for one real, hand-confirmed page (Englewood city, OH) — the identical code and payload render fine locally, so this is not a stale-deploy or a data-correctness problem.**
  - **Issue**: found live 2026-09-13/14 (WO-357). `POST /internal/ingest` for Englewood city, OH's CivicMedia meeting (`us:place:3925396`, "20260908 Council Meeting", 583 real segments) succeeded and returned page id 9715, slug `englewood-oh-20260908-council-meeting`. `GET /m/englewood-oh-20260908-council-meeting` then 500s every time (3 retries, ~2 minutes apart). Ruled out: a stale deploy (`/internal/version` reports production is on `fe06a83`, the exact commit this branch started from); a schema mismatch (`/internal/schema-info` shows the one real mismatch, `meeting_pages.search_tsv`, is a known, already-handled FTS column, not new); wrong/malformed data (`/internal/export/pages?ids=9715` shows a well-formed row with a real `gov_id`, `jurisdiction`, and one transcript version); a broken government hub (`/j/englewood-oh` returns 200); a site-wide outage (four other pages ingested the same day by sibling WOs all return 200). A byte-for-byte identical payload re-ingested into a local `archive.main:app` run (same commit, fresh SQLite) rendered `/m/{slug}` as 200 on the first try. The one confirmed real difference between the two pages compared side by side (Rosetown SK, which renders fine) is real transcript content: Englewood has a non-null `title` and 583 segments of real caption text; Rosetown has a null title and 1,411 segments. That points at something content- or Postgres-dialect-specific in the highlight/rendering path this WO did not have time to isolate further.
  - **Impact**: one real, correctly-identified page is unreachable (500) for any reader who finds the URL. A `dry_run=true` call to `/internal/admin/delete-pages` confirmed the page (title/platform/source all match), but the real delete was blocked by the auto-mode safety classifier, so the broken page is still live in the Archive DB as of this WO's end. `jurisdiction_coverage.csv`'s row for this government was deliberately left untouched (not marked transcribed) so a future session naturally reconsiders it.
  - **Next action**: reproduce against the real production Postgres (or get Sentry/Render log access for the actual traceback — this WO had neither) rather than guessing further; the SQLite-vs-Postgres branch in `archive/db/crud.py`'s `_refresh_meeting_highlight()` (`dialect_insert` from `sqlalchemy.dialects.postgresql` vs `.sqlite`) is the one code path already known to diverge by database and is the most efficient place to start. Once fixed, re-ingest `https://englewood.oh.us/CivicMedia?VID=106` with `gov_id=us:place:3925396` (already hand-checked; see `rtr-business/research/wo357_handcheck.csv`) and delete the current broken page first if it's still there.
  - **Constraint**: don't delete page 9715 without either fixing the underlying bug first or getting explicit sign-off — the transcript itself is real and correct, only the render path is broken.
  - **History**: `BACKLOG_DONE.md`'s WO-357 entry.
