...
Open bugs — real, root cause not settled `[NEEDS-AUDIT]`  (132)
  ...
  Adapter & platform gaps  (43)
    ...
    [NEEDS-AUDIT] Seven corpus-scan pages have a…
...
- **[NEEDS-AUDIT] Seven corpus-scan pages have a township/village-vs-place mismatch that needs a live look, not just a pin.**
  - **Issue**: WO-198's corpus-wide scan (7,836 pages, read-only) found 13 pages whose slug/title says "township" but whose `gov_id` was a `us:place:`/bare `rtr:` id; 6 were pinned or already self-correcting, but 7 need a human/live-page look because either (a) the tenant's own host name strongly implies a real government but nothing was ever extracted (`North Brunswick Township, NJ`'s `northbrunswicktv.cablecast.tv`; `Union Township`, likely Clermont County OH's `utclermont.gov`; two duplicate Washington Township ZBA pages on `wbrw.cablecast.tv`), or (b) the adapter's own extracted jurisdiction disagrees with what the tenant's own domain name says (Newtown, PA's `newtowntownship.civicweb.net` extracts "Newtown (borough), PA" twice; Mantua, OH's `mantuatownshipohio.gov` extracts "Mantua village, OH") — which could be a real shared-portal bleed or an extraction bug, not resolvable from the export alone.
  - **Impact**: at most 7 pages network-wide (a small, already-enumerated population, not an open-ended one) — 4 sit unresolved (`rtr:unknown:...`) with no government at all shown, 3 show a specific but possibly-wrong government.
  - **Next action**: `research/wo198_township_scan.csv` (rtr-business) has the full annotated list with each row's reasoning under `resolution=conductor_review`. For the two host-name cases, confirm the real cousub id from the channel/site and add a tenant-host-level pin (not per-video) in `tenant_overrides.csv`. For the two content-disagreement cases (Newtown, Mantua), open the actual civicweb/township page to see whether it's a real shared portal or a bug before touching anything.
  - **Constraint**: don't pin any of these from the title/slug alone the way the other 6 were — that's exactly the class of case this WO's own resolver fix was built to stop being confident about.
  - **History**: `BACKLOG_DONE.md`, WO-198, 2026-09-11; `rtr-business/research/wo198_township_scan.csv`.
