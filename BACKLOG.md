...
Ship next — root cause known, fix settled `[JUST-DO-IT]`  (60)
  ...
  Coverage registry: per-state view and other dashboard additions…  (9)
  ...
  PrimeGov has a real, structured `committeeId` field — but no…  (4)
    ...
    [JUST-DO-IT] CLAUDE.md's sample-sheet note calling Whitehall, OH's…
...
- **Issue**: ClerkBase's adapter returns names like "Scituate Town Council" with no state, but the tenant slug carries it (`clerkshq.com/Scituate-ri`, `/Pawtucket-ri`). Found by WO-1057's archive comparison.
- **Impact**: 5 of the 10 archived ClerkBase pages resolve to "unresolved" (Scituate, West Warwick, Cumberland, Pawtucket, South Kingstown School Committee), all in RI.
- **Next action**: in `clerkbase.py`, append the slug's two-letter state to the jurisdiction when the name has none, then rerun the archive comparison for `clerkshq.com`.
- **Constraint**: the slug's place part is not always the government's name (`southkingstownschools-ri`), so take only the state from it.
- **History**: `BACKLOG_DONE.md` WO-1057.
...
- **Issue:** The big state-legislature push (BACKLOG_DONE.md, WO-1012) closed Sliq Harmony/Invintus depth, built the Washington/TVW adapter, fixed South Carolina/Tennessee/Nevada, and hand-checked all 49 "nothing found" chambers plus the blocked ones. What's left is smaller and genuinely fragmented — no remaining vendor unlocks more than ~6 chambers.
- **Impact:** Modest and scattered — a handful of chambers each, not a systemic gap.
- **Next action:** (1) A generic (non-Laserfiche) same-path `.vtt` sibling caption lookup in `direct_file.py` — upgrades Massachusetts from video-only to real captions, cheapest fix here. (2) Extract Vimeo's dynamically-fetched event id for Illinois/Georgia — the single biggest remaining unlock (~6 chambers), needs the JS API call this pass didn't capture. (3) Find an ingestible modern Swagit URL for Louisiana Senate (the legacy embedded URL works, `laleg.new.swagit.com` 404s on the paths tried). (4) One more real interaction/hop needed on Idaho, Louisiana House, New Jersey, North Dakota, Ohio, South Dakota, and Texas House's own player hubs (Indiana already confirmed negative — leave it) — several of these, plus Florida/Connecticut's JW Player+HLS and Nebraska/Texas Senate's video.js players, are the same "custom, non-vendor, own-built site" shape as the `generic_fallback.py` `[NEEDS-AUDIT]` entry below (Quincy MA) — a real row/date-aware generic hub walker, if built, could plausibly help both populations at once; check that entry before building anything state-specific here. (5) Invintus depth sweep on the TVW (clientID 9375922947, 1,982 recent events) and CVTV (2917038973, 520+) tenants, same treatment Sliq got. (6) A real government-hub vendor scan for more Invintus tenants (`player.invintus.com`/`eventlisting.invintus.com` embeds) — clientID adjacency is confirmed not to work, unlike Sliq's sequential tenant numbers. (7) Feed the recorded YouTube/Vimeo channel leads (Alabama, Georgia, Kentucky, Maryland, Vermont, Mississippi, New Hampshire) to the YouTube drip lane.
- **Constraint:** New York Senate's Cloudflare human-verification challenge is confirmed real and stays blocked on purpose — never attempt to solve it. Test against real live URLs before building anything, same house rule as always.
- **History:** [BACKLOG_DONE.md](BACKLOG_DONE.md) WO-1012 (2026-09-22, the full push — WO-919/921/922/931/1005/1006/1007/1010/1011 all summarized there). See also this file's `[NEEDS-AUDIT]` "Custom (non-vendor) multi-meeting HTML hub pages" entry (Adapter & platform gaps section) — same underlying problem class, found independently at Quincy MA.
...
- **[JUST-DO-IT] WO-1004 built a domain-health check for the registry's `domain` field, but it hasn't been run yet — needs a machine with `rtr-business` checked out.**
  - **Issue**: two separate hand audits found the registry's `domain` field wrong or stale on a real slice of rows. WO-347's 60-government audit (15% of a 40-row sample) found stale/dead domains: Emigration Canyon township UT's recorded link actually belonged to a different government sharing the same platform, and the domain itself now redirects elsewhere; Floydada TX and Foristell MO's domains migrated to a new TLD (real content, just moved); Wheaton WI's domain now links to an unrelated tourism bureau's YouTube channel; Elmore City OK's domain was repurposed by a laser-engraving company; Leamington UT's `.gov` domain links to an unrelated ghost-sightings site. WO-145's 25-government under-5,000-population pilot (24%) found a different shape — the domain is real and live, just belongs to a different, usually bigger, government: Ventura city IA → Ventura County CA's tenant; Flemington borough NJ → Hunterdon County NJ's tenant; Crystal River city FL → Citrus County FL's tenant (one candidate on it even resolved as a third city, Inverness FL); Jefferson city OR → Jefferson County OR's tenant; Kearny County KS → an unrelated Town of Kearny, NJ; Hometown city IL → a vendor's own demo tenant, not a real second government. Both times the problem surfaced only because a human hand-audited a sample of pipeline output after the fact — nothing flagged it on its own until this WO.
  - **Impact**: if either rate holds across the registry, a meaningful slice of rows are pointed at a domain that will never produce a real page for that government, permanently blocking any future sweep from finding one. The wrong-tenant shape is the more dangerous of the two: it already caused one real wrong-government ingest (WO-145's Crystal River row briefly took on a real Citrus County meeting; caught and deleted the same session).
...
- **[JUST-DO-IT] CLAUDE.md's sample-sheet note calling Whitehall, OH's CivicClerk tenant "agenda-only" is stale -- it now posts video on every recent meeting.**
  - **Issue**: CLAUDE.md's "Working conventions" bullet (dated 2026-08-08) lists "Whitehall OH (CivicClerk, agenda-only)" as a sample. Checked live 2026-09-23 while building WO-1024's Meeting Finder: `whitehalloh.api.civicclerk.com/v1/Events` shows `hasMedia: true` on every real meeting from 2026-07-28 through 2026-09-22 (10 of 10 checked) -- `app/platforms/meeting_finder`'s own live check resolved `whitehalloh.portal.civicclerk.com/event/325/media` to a real tier-3 video (152 minutes).
  - **Impact**: none functionally -- this is a doc-only staleness, not a code bug. It does mean a future session looking for a real agenda-only CivicClerk sample shouldn't reach for Whitehall.
  - **Next action**: update CLAUDE.md's sample-sheet bullet to drop or replace the Whitehall "agenda-only" claim; find a currently-agenda-only CivicClerk tenant if one is still needed as a sample.
  - **Constraint**: none.
  - **History**: found and reported in WO-1024's final report (2026-09-23, `docs/MEETING_FINDER.md`'s "Interface details WO-1024 settled" section).
...
- **Issue:** The improved CivicPlus hub walk lives in `scripts/adhoc_civicplus_pipeline.py`'s `homepage_civicclerk_fallback()` (PR #1362, 2026-09-23): it runs `app/platforms/passive_verify.py`'s `_civicplus_walker()` (AgendaCenter categories, video-hub nav links, `Calendar.aspx?EID=` events), then a homepage vendor-link scan, with a YouTube channel only as a last resort. `app/platforms/civicplus.py`'s own `resolve()` uses none of it. Cablecast, Town Hall Streams and Castus hub URLs now resolve to a real meeting inside their adapters (#1362, #1363); CivicPlus does not.
- **Impact:** A CivicPlus hub pasted into the site, or resolved by any caller other than that script, misses meetings the sweep path finds.
- **Next action:** Move the walk into `civicplus.py` (calling `_civicplus_walker()` from `app/`, not the script) so `resolve()` on a CivicPlus hub returns a real meeting like the other three adapters, and have the script call the adapter. Verify on the real tenants the script's tests use.
- **Constraint:** Meeting Finder (docs/MEETING_FINDER.md) reaches the same walker through its List phase, so this fix is for `/api/resolve` and other adapter callers; don't make Meeting Finder depend on the script.
- **History:** Flagged by the session that built #1362 (2026-09-23).
...
- **Issue**: WO-1065 makes the Invintus resolver spot captions carried inside the video, and the worker extract them. Pages ingested before that deploy still say "No captions found" and don't carry the note the worker looks for.
- **Impact**: any archived Invintus page with no transcript and captions in its video (5 of 11 sampled meetings on 2026-09-25) waits in the ordinary oldest-first queue for a Whisper run instead of getting its own captions. Not yet counted.
- **Next action**: after the resolver, Archive and both workers are deployed, count archived `platform='invintus'` pages with no good transcript, then re-resolve and re-ingest them one at a time (each probe reads about 1 MB). The workers take it from there.
- **Constraint**: Render shell only, never a laptop against production. One request at a time per host; stop on 403 or 429.
- **History**: `BACKLOG_DONE.md` WO-1065.
...
### Mad River Valley TV's three playlist pins name no TelVue org token `[HUMAN]` `[EXAMPLE]`

- **Issue**: pins `playlists/4259`, `4260`, `4261` (Waitsfield, Warren, Fayston, VT) carry no org token, and none was found in fixtures, pins, research files or mrvtv.com (which embeds TelVue's other product, connect.telvue.com). So `tests/test_tenant_key.py` cannot place them in a tenant, and they are listed there as known exceptions.
- **Impact**: MRVTV cannot be added to `MULTI_GOVERNMENT_TENANTS` (app/utils/tenant_key.py), so if MRVTV's token ever appears unpinned, its adapter's per-meeting name would be trusted.
- **Next action**: find one real MRVTV `videoplayer.telvue.com/player/{token}/...` URL, check the three playlists answer under it, then add the token to `MULTI_GOVERNMENT_TENANTS` and drop the three pins from `KNOWN_UNMAPPED_PINS`.
- **Constraint**: none.
- **History**: `BACKLOG_DONE.md` WO-1056 and WO-1057.
...
- **[NEEDS-AUDIT] Meeting Finder's meeting-evidence rule (WO-1041) is slightly strict: it demoted 8 of 34 hand-approved real meetings.**
  ...
  - **Next action**: (1) **Confirmed bug, fix first:** the live check in `resolve.py` does not pass the video's own filename to `assess_meeting_evidence()` -- run 3 demoted Bellerive Acres MO's 28-minute "(2026-09-22) City Council Special Meeting.mp4" and Bound Brook NJ's "Reorganization-Meeting.mp3" as "no meeting evidence". Pass the filename (and a Google Drive file's title) too. (2) Run 3's measurement (2026-09-24): of 600 weak leads, 17 had meeting words in the title and 6 of those were real meetings; a random 20 without meeting words were 19 promos + 1 can't-tell -- so the demotion is right for titleless short files, and the misses come from (1) and from the video gate's `hand_check_kind` rejecting real board meetings (Sag Harbor SD "Board of Education Business Meeting", SFUSD "Board of Education").
...
- **[NEEDS-AUDIT] `detect_platform()`/`parse_vimeo_video()` never recognizes a Vimeo "Event" (livestream) URL — never resolvable today, by design, not just unbuilt.**
  ...
  - **Impact**: corrected 2026-09-24 (WO-1046) — this entry previously implied Suffolk County NY (`us:county:36103`) was stuck reporting `youtube-lead-only` because of this gap. That's no longer true: Suffolk's real answer was never the Event embed, it was the 14 real `vimeo.com/showcase/{id}/embed` archives on the SAME page (already parseable before this entry was filed) — a separate resolve-time bug (source-url tracking + a hop/scan dedup race, both fixed in WO-1046) was what actually blocked them. Suffolk now resolves to `embed-restricted` (a confirmed real meeting, domain-restricted by its owner — see `BACKLOG_DONE.md`'s WO-1046 entry). The live-event-VOD question itself remains genuinely open, but no known real government is currently blocked on it.
  - **Next action**: only worth building if a real government surfaces whose ONLY video is behind a Vimeo Event's own VOD replay (none confirmed as of 2026-09-24) — confirm what that page's own HTML/API offers on 2-3 real examples first, per the "test against a real URL first" rule.
  - **Constraint**: resolver-level (`app/platforms/vimeo.py`/`base.py`), not a Hop/Scan link-ranking bug.
  - **History**: found live verifying WO-1044's Suffolk County NY fix; narrowed and Suffolk's own resolve unblocked in WO-1046 — see `BACKLOG_DONE.md`'s WO-1044 and WO-1046 entries.
...
- **[NEEDS-AUDIT] Newer meeting exports show pages the wrong-page worklist expected to find unchanged: page 2504 has lost `us:county:27007` and page 3643 is no longer flagged.**
  ...
  - **Constraint**: do not edit the export or the worklist to make old assertions pass; the two tests that read the 2026-09-21 export now skip on any other export (WO-1081).
  - **History**: `BACKLOG_DONE.md`, WO-1081 (2026-09-26), which fixed the tests themselves.
...
- **[NEEDS-AUDIT] Two tests failed only in full local runs on Ryan's Mac, never in CI or a cloud container; the one Mac-only difference found is that collecting the suite there imports rtr-discovery.**
  ...
  - **Impact**: none today. WO-1081 changed both tests so they no longer depend on the shared test database or on a fixed wait. But an import that changes the import path for the whole run can still affect other tests on the Mac.
  ...
  - **History**: `BACKLOG_DONE.md`, WO-1081 (2026-09-26); first seen 2026-09-10 (WO-166).
...
- **[NEEDS-AUDIT] Four more tests fail when the test files run in a different order: rows left by one file in the shared test database change another file's results.**
  ...
  - **History**: found running the suite in shuffled order for WO-1081, 2026-09-26.
...
- **[NEEDS-AUDIT] `[EASY]` Some tests still make real DNS lookups; they pass without network today, but the suite is not fully offline.**
  ...
  - **History**: found in WO-1081's offline scan (`BACKLOG_DONE.md`).
...
- **[NEEDS-AUDIT] The YouTube fetch guard does not stop a YouTube request made through an HTTPS proxy.**
  ...
  - **History**: found running the full suite for WO-1081, 2026-09-26.
...
- **[NEEDS-AUDIT] The coverage registry's `domain` field maps a small government to a completely different government's tenant far more often than WO-142's original sample suggested — 6 of 25 (24%) in WO-145's under-5,000-population pilot, not WO-142's ~14%.**
  ...
  - **Next action**: `wo145_api_first_sweep.py`'s `_state_or_kind_conflict()`/`_title_place_conflict()` (state-name conflict, county-vs-municipality keyword, and a specific different place-name in either the adapter's resolved jurisdiction/meeting_body or the resolved title, checked against the row's own name tokens) is the working reference implementation — port it into a shared helper (`app/utils/gov_registry/` or `scripts/`) that any future sweep imports, rather than each one re-discovering this the hard way. The registry-wide backfill/detection half of this (re-derive each `domain`'s actual resolved government from a real fetch) is now WO-1004 (see this file's "Ship next" section) — its new outcome bucket flags this failure shape too, since a wrong-tenant match won't name the right government either.
...
- **Next action:** WO-1053 (dry run, `BACKLOG_DONE.md`) built the seed file (`app/utils/jurisdiction_data/regional_tv_hubs.csv`, 11 hubs) and `scripts/hub_harvest.py`, and confirmed Nashwauk MN and Wilder KY both have their own real, dedicated section on their hub (Iron Range TV Cablecast, Campbell County KY Cablecast). What's left: (1) actually ingest the 18 confident, not-yet-covered matches its dry run found (its own `ingest_queue_plan.csv`, not committed to the repo — rerun the script for a fresh copy); (2) Meeting Finder itself (`app/platforms/meeting_finder/`) still needs Hop/List changes to treat a hub link as a governing-body link and pick a government's own section automatically at RUN time, which this WO deliberately did not touch (out of its owned files); (3) TVCTV and Pierce County TV's own newest-meeting video still isn't reachable by this pass (TVCTV's real show API has no confirmed per-site filter; Pierce County TV's is behind a further JS widget on its own non-YouTube site) — both need more real-page investigation before they can produce a confident match at all.
- **Constraint:** YouTube-hosted hubs (Pierce County TV) are drip-only: record them as leads with per-government pins, never fetch YouTube off the drip Mac. WO-1053's harvester never calls its own network session for Pierce County TV at all (tested).
- **History:** Meeting Finder overnight run follow-ups, 2026-09-24 (conductor session 93b5f9ae); research-file reason `hosted-on-regional-site` (ENUMERATION_METHODS.md §23); WO-1053 (2026-09-24, dry run, `BACKLOG_DONE.md`) built the seed file and harvester and confirmed Nashwauk/Wilder.
