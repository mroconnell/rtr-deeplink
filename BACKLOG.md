...
- **[HUMAN] Fond du Lac County, WI — decide an authoritative host pin for `fonddulac.granicus.com` (WO-1000, OpenPublica).**
  - **Issue**: the tenant is the County Board of Supervisors, but the name ladder resolves "Fond du Lac, WI" to the city (`us:place:5526275`) before a `fallback` pin is consulted, so a `fallback` pin for `us:county:55039` is inert (checked 2026-09-21 with `resolve_government()`).
  - **Impact**: one new-to-Archive county government (no page today) is held out of the tier 3 queue; queuing it as-is would file the meeting under the city.
  - **Next action**: Ryan decides. If yes, add `fonddulac.granicus.com,,us:county:55039,authoritative,...` to `tenant_overrides.csv` and queue the probed meeting (`https://fonddulac.granicus.com/MediaPlayer.php?view_id=1&clip_id=1028`, 1883 s, saved in the research folder's `tier3_pending.csv`).
  - **Constraint**: an authoritative pin covers the whole host. Fine here (the tenant only carries County Board meetings, checked from its listing 2026-09-21), but check first that the tenant has no city meetings.
  - **History**: `rtr-business/research/openpublica_2026-09-21/README.md` and `ENUMERATION_METHODS.md` §393.
