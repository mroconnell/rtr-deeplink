...
Open bugs — real, root cause not settled `[NEEDS-AUDIT]`  (186)
  ...
  [NEEDS-AUDIT] `[BIG]` This codebase has no Google Drive video…
  ...
  [NEEDS-AUDIT] Two real government YouTube channels organize meetings…
  ...
  Adapter & platform gaps  (52)
    ...
    [NEEDS-AUDIT] `scripts/backfill_gov_id.py`'s "hub slugs retired"…
...
- **[EASY] A discarded trim/split tail that starts with a lowercase "and"/"for" still short-circuits straight to "bleed" before the entity-suffix protections ever run — 2 of the 12 real WO-299 names WO-300 didn't fix.**
  - **Issue**: WO-300 (2026-09-12) fixed 10 of the 12 real Utah PMN "Entity" names WO-299 found wrongly resolving to Utah County or Ogden city (see `BACKLOG_DONE.md`'s WO-300 entry for the mechanism and the other 10). Two remain wrong: `resolve_government("Box Elder County and Perry City Flood Control Special Service District, UT")` still resolves to Box Elder County (`us:county:49003`) and `resolve_government("Grand County Service Area for Castle Valley Fire Protection, UT")` still resolves to Grand County (`us:county:49019`). Both discarded tails ("and Perry City Flood Control Special Service District", "for Castle Valley Fire Protection") start with a lowercase word that ISN'T "of" ("and"/"for"), so `jurisdiction_enrich._looks_like_bleed()`'s general lowercase-word signal returns `True` immediately — before `_ends_with_known_entity_suffix()` (which already protects "district"-ending tails) or WO-300's new "of"-specific allowlist ever get a chance to run.
  - **Impact**: same shape as the already-fixed cases — a real, separate special-district government gets silently filed under the county/city its name happens to start with.
  - **Next action**: widen WO-300's "of"-specific pre-check in `_looks_like_bleed()` (`app/utils/jurisdiction_enrich.py`, the `_ENTITY_NAME_OF_SUFFIX_PHRASES`-guarded block) to also cover a tail whose only non-"of" lowercase words are "and"/"for" acting as connectors between Title-Case phrases — grounded in these two real cases — or add both full tails as literal entries the way `_ENTITY_TYPE_SUFFIX_PHRASES` already does for "committee of adjustment"/"committee of the whole". Re-run `tests/test_gov_registry.py`'s `test_name_merely_containing_a_place_name_does_not_resolve_to_it` with these two added before considering it closed.
  - **Constraint**: verify against the WO-300 blast-radius method (run the fixed ladder over every distinct raw `jurisdiction` string in the Archive export, compare before/after with the same tenant host context, never against the raw stored `gov_id` — that conflates unrelated code/pin drift) before merging any further widening.
  - **History**: `BACKLOG_DONE.md`'s WO-299 and WO-300 entries.
...
- **[NEEDS-AUDIT] `[BIG]` This codebase has no Google Drive video adapter, and small-town governments really do post real meeting recordings there.**
  - **Issue**: found live 2026-09-12 (WO-264) — Kemmerer city, WY shares three Drive files literally titled `City Council Meeting - 2026/08/10 16:52 MDT - Recording` from its own site, and Walbridge village, OH has a Drive folder named `Meeting Recordings` alongside its Agendas/Minutes/Packets folders. Resolving one live through the real production pipeline (`app.platforms.base.detect_platform`/`get_finder`) confirms `detect_platform()` returns `"unknown"`, `GenericFallbackAssetFinder` correctly extracts the real title from Drive's viewer page, but `result.video_url` comes back `None` — Drive's viewer page doesn't expose a direct playable media URL in static HTML (it's loaded via JS/API calls), and Drive's confirm-download flow (plus a virus-scan interstitial on larger files) needs real handling this codebase doesn't have.
  - **Impact**: at least these two real, confirmed governments have an on-mission meeting recording that cannot become an Archive page today, and 26/52 of WO-264's `video-candidate` outcome rows were Drive/Dropbox links — most are agenda PDFs (not video), but this shows the pattern is not a one-off.
  - **Next action**: build a Drive adapter that resolves a `drive.google.com/file/d/<id>/view` URL to a real downloadable/streamable media URL (Drive's own API, an unauthenticated direct-download link pattern, or a documented workaround for the large-file virus-scan interstitial), using Kemmerer's three real files and Walbridge's folder as the first live samples per this repo's "test against a real URL first" rule.
  - **Constraint**: don't assume Drive folder LISTINGS (as opposed to a single file's view page) are scrapable via plain HTTP — a live check this WO found Walbridge's `Meeting Recordings` folder page renders its contents via JavaScript, so a folder-listing walk needs a different (headless, or Drive API) approach than a single-file resolve.
  - **History**: `BACKLOG_DONE.md`'s WO-264 entry; `rtr-business/research/ENUMERATION_METHODS.md` §303.
- **[NEEDS-AUDIT] This codebase has no adapter for a direct video file hosted on a government's own domain or on Dropbox, distinct from the Google Drive gap above.**
  - **Issue**: found live 2026-09-12 (WO-284) — `bulk_ingest.py --dry-run` resolves 5 real, clearly-labeled meeting recordings as `platform=unknown, segments=0`: Palisade town, CO (several dated `Zoom-Video_Board-of-Trustees_*.mp4` files on its own domain), Dundee city, OR (`GMT20221129-023258_Recording_1280x720.mp4`, a Zoom-export-shaped filename), Cayuga Heights village, NY, and Enterprise city, OR (Dropbox-hosted `2025-09-09-City-Council-Recording.mp4` and several other real, clearly-labeled recordings in the same Dropbox share).
  - **Impact**: at least these 4 governments (5 files) have a real, on-mission meeting recording that cannot become an Archive page today; the pattern likely recurs anywhere a small government hosts its own Zoom export instead of using a vendor platform.
  - **Next action**: build a generic direct-media-file adapter (`.mp4`/`.mov`/`.webm` served with a normal `Content-Type: video/*` from any host, plus Dropbox's own direct-download URL shape `?dl=1`) that `detect_platform()` can recognize by content-type/extension rather than by a known vendor host.
  - **Constraint**: don't widen this to Google Drive — that's the separate, harder gap in the entry above (Drive's viewer page needs auth/JS handling a plain file URL doesn't).
...
- **[NEEDS-AUDIT] Two real government YouTube channels organize meetings into playlists (by committee or by year) with no flat "Council Meeting" video ever appearing directly in the channel's own Videos tab.**
  - **Issue**: found live 2026-09-12 (WO-279) — Watertown city, SD's channel groups every meeting into per-year playlists ("2026 City Council Meetings", "2025 City Council Meetings", etc.), so a flat `playlistend=40` listing of the Videos tab returns only the playlist containers themselves (duration 0, no individual meeting videos) until one specific playlist is expanded a level further. Groton city, CT's channel goes further: meetings are split across dozens of PER-COMMITTEE playlists (its real governing body is a Representative Town Meeting with committees like "Groton RTM Public Safety Committee", "Groton RTM Education Committee") with no single "the meetings" playlist to expand.
  - **Impact**: any channel-scan sweep that reads only the flat Videos-tab listing (this WO's own method, and WO-134/WO-235/WO-247's shared technique) will see zero real candidates on a channel organized this way, misreading "no video found" as the conclusion when the video actually exists one playlist-expansion hop away. Watertown's real video was found and ingested this WO by hand-expanding the one obviously-relevant playlist; Groton's was not (no single playlist stood out as "the" governing-body meeting, and expanding all ~40 committee playlists by hand was out of scope for one government).
  - **Next action**: teach the shared channel-listing helper (`_yt_dlp_listing_sync`/`yt_dlp_listing` in `scripts/wo235_channel_pilot.py`, the pattern this WO's own `scripts/wo279_second_look.py` copied) to detect a listing made up entirely of zero-duration playlist entries and automatically expand the most promising one (title containing "council"/"meeting"/the government's own governing-body name) one level, rather than reporting zero candidates.
  - **Constraint**: only 2 real examples on file (Watertown SD, Groton CT) — Groton's committee-fragmented shape may need a different, narrower method (matching the government's actual body name against committee playlist titles) than Watertown's simpler by-year shape; don't build one fix assuming it covers both until a third example clarifies which shape is more common.
  - **History**: `BACKLOG_DONE.md`'s WO-279 entry, 2026-09-12; `rtr-business/research/ENUMERATION_METHODS.md` §304.
...
- **[NEEDS-AUDIT] `scripts/backfill_gov_id.py`'s "hub slugs retired" count never fires for the exact shape that caused the original alias-chain bug (a KEYED minted government re-keyed to a different KEYED government) — only for a blank/`rtr:unknown:` page gaining identity for the first time.**
  - **Issue**: found live 2026-09-12 building WO-293's alias-chain repair. `owned_slugs.add(old_hub)` runs unconditionally for every row that HAS a `current_gov_id` (guarded only against blank/`rtr:unknown:`), even when that same row's own `hub_moves` entry is the thing abandoning `old_hub` — so a minted government's slug marks itself "still owned" via the very page that's leaving it. Reproduced directly: seeding one page with a freshly-minted `rtr:` id and a jurisdiction string that a fresh, unrestricted resolve puts on a different real government, then running `backfill_gov_id.py --apply` against it, prints `hub slugs retired : 0` even though the minted government ends the run with zero pages. This is exactly the Lake Havasu shape (`rtr:us:az:lake-havasu` re-keyed onto `us:place:0439370` by WO-251's fix) — and matches the historical evidence: the one real precedent where "hub slugs retired" printed a nonzero count and a human added the aliases by hand (`BACKLOG_DONE.md`, 2026-09-10, "11 hub slugs retired") was entirely blank/`rtr:unknown:youtube.com` pages gaining a channel pin for the first time, never a keyed-to-keyed re-key.
  - **Impact**: WO-293's new `write_retirements()` hook (called from this same report, `archive/utils/hub_aliases.py`) is exercised correctly and automatically for the blank/`rtr:unknown:` → keyed case (verified by a real DB integration test, `tests/test_hub_alias_repair.py::test_backfill_gov_id_apply_writes_an_alias_when_it_retires_a_slug`), so most future retirements get their alias written automatically. But a future WO-251-shaped incident (a resolver fix correctly re-keying an already-minted government) would still silently produce a broken alias chain exactly the way `lake-havasu-az` did, because the report never flags it as retired in the first place — there's nothing for the new hook to act on.
  - **Next action**: `owned_slugs` needs to reflect END-OF-RUN ownership, not "did this row show `current_gov_id` at read time" — e.g. track, per `current_gov_id` touched this run, whether at least one of ITS pages is NOT part of a hub_move (still `old_hub == new_hub`, a `manual_override` skipped earlier, or genuinely outside `--hosts`/`--limit` scope); only then is `old_hub` still owned. A `hub_slug_table.page_count(session, gov_id)` query (already used by the freeze gate) after `args.apply` commits is probably the simplest correct answer, even though it costs one query per candidate retired government rather than staying purely in-memory.
  - **Constraint**: don't "fix" this by just removing the `owned_slugs.add(old_hub)` line for a moving row — a slug legitimately stays owned when ONE of a government's several pages moves away and others don't; the fix has to distinguish "every page under this gov_id is leaving" from "some are, some aren't," not assume the former.
  - **History**: `BACKLOG_DONE.md`'s WO-293 entry (the alias-chain repair this was found while building); `BACKLOG_DONE.md`'s WO-251 entry (the original incident this gap would reproduce) and its 2026-09-10 "11 hub slugs retired" entry (the one confirmed case where this DOES fire correctly today).
