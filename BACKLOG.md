...
- **[HUMAN] Apply the reviewed wrong-page sheet, `reports/wrong_page_worklist.csv` (48 rows): merge, deploy the Archive and the resolver, then run the tool on the Archive's Render shell.**
  - **Issue**: WO-934 built `scripts/repair_wrong_pages.py` and the sheet, and Ryan decided the rows on 2026-09-21. If every live page still matches, a run writes 38 re-keys (20 that needed no decision, 13 school boards Ryan approved, and 5 pages that move to newly minted governments) and, with `--allow-deletes`, 3 deletes (6114, 6101, 6830). It skips 8 rows: 2504 (approved, but no registry id to write; see Next action), 3453, 3367, 6906 and 7086 (all waiting), and 6119 and 6218 (Ryan kept them).
  - **Impact**: 41 live pages show the wrong government (or none) and 3 show a video that is not a meeting, until the run happens. The five mint rows fail until the Archive is deployed with the minted governments: it refuses an id it does not know, and the tool stops on that. The Render shell only sees files from the deployed commit.
  - **Next action**: (1) merge the PR, then deploy the Archive and the resolver. (2) On the Archive's Render shell: `python scripts/repair_wrong_pages.py check reports/wrong_page_worklist.csv`; then `... run reports/wrong_page_worklist.csv` (a dry run, nothing changes; read it); then `... run reports/wrong_page_worklist.csv --apply --batch-size 5`, again and again until nothing is left to write; then `... run reports/wrong_page_worklist.csv --apply --allow-deletes --only-ids 6114,6101,6830`. (3) Page 2504 (Minnesota PUC filed under Beltrami County; Ryan: move it to unresolved like page 5816, do not delete): `backfill_gov_id.py --hosts minnesotapuc.granicus.com` does NOT move it (it recomputes "Beltrami County, MN"), and the override route needs a registry id. What works is a re-push under the commission's own name, on the resolver's Render shell: `python scripts/repoint_page.py "https://minnesotapuc.granicus.com/player/clip/2573" "https://minnesotapuc.granicus.com/MediaPlayer.php?view_id=2&clip_id=2573" --dry-run`, then the same command without `--dry-run`. (4) Re-run the screen (read-only): `python scripts/wrong_page_screen.py --from-archive --worklist reports/wrong_page_worklist.csv --out /tmp/wrong_page_screen.csv`.
  - **Constraint**: the override route also writes draft `tenant_overrides.csv` lines to a file on the Archive machine; the tool copies them into its log. Commit none for a shared host (a city's Granicus, Cablecast or CivicClerk site that also carries its school board). Copy the log out before any deploy; a deploy wipes `/tmp` and the shell's working files. Ryan still decides pages 3367 (Derry, NH) and 3453 (Hopkins on Vimeo), and whether the `solanocoe.granicus.com` pin should be authoritative (its stateless name still resolves to Solano County; see `BACKLOG_DONE.md` WO-934).
