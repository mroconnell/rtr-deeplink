...
Ship next — root cause known, fix settled `[JUST-DO-IT]`  (51)
  ...
  PrimeGov has a real, structured `committeeId` field — but no…  (3)
    [JUST-DO-IT] `archive_client.send_search_alerts()` has no retry, so…
    [JUST-DO-IT] `list_all_page_slugs()` doesn't exclude…
    [JUST-DO-IT] The registry's `domain` field is wrong or stale on a…
...
- **Issue:** WO-919 (2026-09-20) ran the passive check and access ladder on all 99 chamber rows of the 2026-09-17 recon (`rtr-business/research/50_state_legislative_media_recon.md`; 49 two-chamber states plus unicameral Nebraska). 7 pages went live and 1 chamber already had pages, so 91 rows had no page that morning. Since then: the Sliq Harmony adapter was built (WO-921) and six Sliq pages are live (AR, CO, DE, NM, OK, WV; each answered HTTP 200 on 2026-09-21, backfill-dry-run confirmed correctly keyed on 2026-09-22 by WO-1006); PR #1260 queued five video-only meetings (Kansas Sliq and four Invintus, two Oregon and two Wisconsin), all still in `scripts/tier3_auto_transcription_queue.txt` on 2026-09-21; and the Invintus hub walker exists (WO-922). WO-1006 (2026-09-22) closed both open Sliq questions from this entry: the sibling hosts `sg002-harmony.sliq.net`/`sg004-harmony.sliq.net` are confirmed mirrors of `sg001`'s tenant catalog, not separate tenant pools (same tenant number returns byte-identical content and the same 404 on all three); and Oklahoma's Senate tenant is 00282 (found by trying nearby tenant numbers, one meeting ingested), while West Virginia's House of Delegates is confirmed NOT on Sliq at all (its own site links a YouTube channel for House video; only the Senate uses Harmony) — not a missing tenant. The same tenant-number sweep also found five brand-new state legislature tenants nobody had a page for: Maine and Iowa (video only, now queued for tier-3), Nevada, Missouri and Virginia (captioned, one page each ingested) — see `rtr-business/research/wo1006_handread.csv`/`wo1006_ingested.csv` and `app/platforms/sliq_harmony.py`'s module docstring. The 91 is still stale and was not recounted: the Sliq pages are filed under the state with a committee as meeting body, and the ingest CSVs record no chamber for them, so how many of the 99 rows they satisfy needs each committee read by hand. The 91 of 2026-09-20 split as: 17 on a known video vendor (11 Sliq, 4 Invintus, 2 TVW); 49 where the automatic check found nothing; 5 YouTube channel leads (AR House, MT, WY); 3 with no online video archive (NC, MO Senate); 2 South Carolina direct-mp4 finds that cannot be queued yet; 15 blocked (3 TLS certificate failures: IL, MI, MS; 2 timeouts: CT, NH; NY Senate challenge, stopped; TN and NV Granicus tenants 403 plain HTTP and 200 with browser headers) — all three of those last items are now closed by WO-1005 (2026-09-22, PR #1341): `direct_file.py` now accepts South Carolina's octet-stream `.mp4`s (both chambers queued for tier-3); `granicus.py` got a browser-headers retry after a 403 (Tennessee's Senate and House are both **ingested with real captions**, live once deployed); Nevada's Granicus block turned out unreproducible live and is moot anyway — its real, working video source is Sliq Harmony (tenant 00324). WO-1011 (2026-09-22) hand-checked the 49 "nothing found" rows and re-verified the remaining 6 blocked chambers (`rtr-business/research/wo1011_report.csv`, one row per chamber with the real platform/vendor identified). Five of the six blocked chambers turned out to be **transient, not real blocks**: IL, MI, MS (TLS "failures") and CT, NH (timeouts) all answer a normal 200 today, each with a real video vendor now identified (Vimeo + an Amazon IVS live channel for Illinois; an own player for Michigan House and Castus — already-supported, but gated by a bot-challenge on this specific tenant subdomain — for Michigan Senate; a shared YouTube channel for Mississippi; a self-hosted JW Player + HLS setup for Connecticut; two YouTube channel leads for New Hampshire, though the two specific videos linked from its hub turned out to be off-mission explainer clips, not real meetings). Only New York Senate's Cloudflare challenge is confirmed still real (a live `cf-mitigated: challenge` response header) and was correctly left alone. Of the 49 "nothing found" rows, 11 (Arkansas Senate, Iowa, Maine, Missouri House, Oklahoma Senate, Pennsylvania, Virginia) were already resolved by WO-1006/WO-1007 before this pass even started — re-derived from the current `jurisdiction_coverage.csv` rather than re-checked live. Of the rest: **2 real meetings were ingested** (Hawaii House and Senate, via the Legislature's own Olelo Granicus channels one hop off its hub — the automatic check missed them because they're linked from the hub, not embedded on it), **1 real video was queued for transcription** (a Massachusetts House floor session — a direct mp4 with real WebVTT captions on Azure blob storage, but `direct_file.py` has no generic same-path `.vtt` sibling lookup outside its Laserfiche-specific case, so it queues video-only rather than ingesting with captions), and the remainder split into YouTube/Vimeo channel leads not fetched further (Alabama, Georgia, Kentucky, Maryland, Vermont — Kentucky's own `legislature.ky.gov` links a confirmed first-party "LRC Capitol Connection" channel, distinct from the KET owner-body channel already flagged), newly-identified self-hosted/own-built platforms with real video confirmed but too few examples to adapt from yet (Florida and Connecticut: JW Player + HLS; Nebraska: a custom "Legislature Video Library" React SPA; Texas Senate and Utah: video.js players, Utah's backed by a real, own-hosted Wowza HLS stream at `stream1.utleg.gov`), and several still-inconclusive own-player hubs that need one more real interaction/hop (Idaho, Indiana [confirmed negative, re-verified], Louisiana, New Jersey, North Dakota, Ohio, South Dakota, Texas House). See that report's own platform-grouping section for which vendor would unlock the most additional chambers if built.
...
- **Next action:** (1) Sliq: sweep more meetings per state — each tenant holds many more than the one meeting per state read so far (WO-921 read ~12-14 per tenant; WO-1006's five new tenants got 3-9 reads each, thinner than that). Iowa/Maine/Kansas publish no captions, so their meetings go to the transcription queue (queued, not yet transcribed). Pennsylvania's `jurisdiction_coverage.csv` question is closed (WO-1007, 2026-09-22): PennDOT was already its own government since WO-201, the research file was just stale, now corrected — no re-key needed. Two more real Sliq tenants were found and left alone on purpose: Government of Nunatsiavut (00280) and Bermuda Parliament (00290), both outside this repo's US-jurisdiction scope. (2) Invintus: the four queued meetings need transcription; then run the walker (`invintus.list_recent_events()`, `passive_verify._invintus_walker`) on the other Invintus tenants. (3) TVW adapter for Washington (`app/platforms/` has none) — the one vendor left with rows and no adapter at all. (4) The 49 "nothing found" rows are now hand-checked (WO-1011, `research/wo1011_report.csv`) — remaining real work from that pass: extract a real archived (not live-only) Vimeo event id for Georgia/Illinois (the id is fetched dynamically via a JS API call this pass didn't capture); find an ingestible Swagit URL for Louisiana Senate (the modern `laleg.new.swagit.com` customer domain 404s on the paths tried); one more real interaction/hop on Idaho, Indiana (confirmed negative), Louisiana House, New Jersey, North Dakota, Ohio, South Dakota, and Texas House's own player hubs; a generic (non-Laserfiche) same-path `.vtt` sibling caption lookup in `direct_file.py` would upgrade Massachusetts's queued video-only meeting to real captions without waiting on Whisper. (5) Of the six blocked chambers, five (IL, MI, MS, CT, NH) turned out to be transient and are now unblocked with a real vendor identified each (WO-1011) — only NY Senate's Cloudflare challenge is confirmed still real and stays left alone.
- **Constraint:** hand-check every meeting title against the chamber or committee; at most 2 chambers' worth per state; the recon has more copy/paste rows than the four the plan named (also Arkansas Senate, California Assembly, Louisiana House, Michigan Senate — the row shows the other chamber's site). "Nothing found" means the automatic check found nothing, not that the chamber has no video. Do not quote 91 again until a recount says which committee belongs to which chamber.
- **History:** [BACKLOG_DONE.md](BACKLOG_DONE.md) WO-919 (the check), WO-921 (the Sliq adapter), WO-922 (the Invintus walker); PR #1260 (the queue lines); WO-931 (this rewrite); WO-1006 (2026-09-22, sibling hosts + OK Senate/WV House + five new tenants); WO-1005 (2026-09-22, PR #1341, SC/TN/NV Granicus and direct_file fixes); WO-1007 (2026-09-22, Pennsylvania research-file correction); WO-1011 (2026-09-22, hand-check of the 49 "nothing found" rows and re-verification of the 6 blocked chambers — 2 Hawaii meetings ingested via Granicus, 1 Massachusetts meeting queued for transcription, 5 of 6 blocked chambers unblocked, `research/wo1011_report.csv`).
...
- **[JUST-DO-IT] The registry's `domain` field is wrong or stale on a real slice of rows — two hand audits found 6 of 40 (15%) and 6 of 25 (24%) — and nothing catches this automatically today; WO-1004 adds a "domain doesn't match" outcome to the existing passive-discovery pipeline.**
  ...
  - **Next action**: WO-1004, assigned 2026-09-22. The fetch and identity-check machinery both already exist in the WO-282/283 passive-discovery pipeline — phase 3 (the `wo283_targeted.py` lineage) already fetches every government's homepage, including rows that already have a `domain` on file, and already requires `name_matches()` (the fetched page must name the right city AND state) before counting a candidate as confirmed. The gap: when a domain-on-file row fails that check, it's folded into the ordinary "no candidate confirmed" outcome instead of being called out on its own. Add a distinct outcome bucket for exactly that case, run it over every row with a non-blank `domain`, and write the result to a worklist for hand confirmation.
  - **Constraint**: never blank or overwrite `domain` outright (`CLAUDE.md`'s standing rule) — a confirmed replacement goes to `alternate_domains`/`alternate_urls`, the original stays. Never auto-write the registry from this outcome — same hand-confirm bar WO-347 and WO-145 both used.
  - **History**: `BACKLOG_DONE.md`, WO-347 (2026-09-13, `research/wo347_wrong_domain_findings.csv`) and WO-145 (2026-09-10). See also this file's "coverage registry's `domain` field maps a small government..." entry (Open bugs) for the still-open shared-helper half of WO-145's finding that WO-1004 doesn't cover.
