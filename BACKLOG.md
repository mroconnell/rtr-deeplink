...
Open bugs — real, root cause not settled `[NEEDS-AUDIT]`  (208)
  [NEEDS-AUDIT] Meeting Finder's verdict output never keeps the…
  [NEEDS-AUDIT] `app/platforms/direct_file.py`'s Google Drive check…
...
- **[NEEDS-AUDIT] Meeting Finder's verdict output never keeps the meeting/candidate PAGE url `resolve()` was called with — only the pre-resolve homepage hop trail and the final resolved video URL.**
  - **Issue**: `resolve.py`'s `_try_resolve()` calls `finder.resolve(cand.url)` on `Candidate.url` (the real tenant/platform meeting page), but that `Candidate` is never persisted into `VerdictRow`/the JSONL output — `app/platforms/meeting_finder/verdict.py`'s `CSV_FIELDS`/`VerdictRow` (`models.py`) have `path` (the pre-resolve homepage hop trail) and `result_url` (the final resolved video URL, e.g. a raw `archive-stream.granicus.com/.../*.m3u8` or `cpmedia.azureedge.net/*.mp4`) but no field for the page URL itself. Confirmed live 2026-09-24 building WO-1040 (`scripts/meeting_finder_followups.py`): calling `get_finder(platform).resolve()` directly on a real batch's `result_url`s, Vimeo player URLs resolve cleanly (result_url IS the real input there), but a Granicus/Swagit/IQM2 stream URL returns an empty, useless result (no title, no video_url, no segments — `detect_platform()` matches "granicus" on the CDN hostname, so it never falls through to the generic `direct_file` adapter, and `GranicusAssetFinder.resolve()` needs the real on-demand page, not the raw stream).
  - **Impact**: any downstream consumer of a Meeting Finder verdict (WO-1040's followup script today; presumably any future one) can only feed `result_url` into `bulk_ingest.py`/the tier-3 queue, which — for every platform except Vimeo and whatever bare CDN mp4/mp3 happens to fall through to `direct_file` — will not produce a real transcript. Not a guess: see WO-1040's PR for the real per-platform table.
  - **Next action**: add a `candidate_url` (and ideally `source_page_url`, mirroring `probe_queue_entry`'s own `source_page_url=result.source_url or cand.url`) field to `VerdictRow`/`CSV_FIELDS`, populated from `ResolveResult.candidate.url` at the point `runner.py` builds the row (`result_url=result.video_url` — same spot, `runner.py` lines ~1062/1122/1137/1171). Update `verdict.py`'s `_DATACLASS_FIELDS` assertion and any existing verdict-shape test.
  - **Constraint**: `verdict.py` is explicitly read-only and append/resume-friendly (a rerun skips `input_url`s already in `out_path`) — a new field must be optional/backward-compatible with existing verdicts.csv/jsonl rows that don't have it, not a breaking schema change.
  - **History**: found live building WO-1040 (this backlog edit's own PR); see that PR's `scripts/meeting_finder_followups.py` module docstring for the full per-platform investigation.

- **[NEEDS-AUDIT] `app/platforms/direct_file.py`'s Google Drive check only recognizes the classic share-link shape, not the already-rewritten download URL it produces internally.**
  - **Issue**: `is_direct_file_url()`'s `_DRIVE_FILE_ID_RE = re.compile(r"drive\.google\.com/file/d/([\w-]+)")` matches only Drive's classic viewer link (`drive.google.com/file/d/<id>/view`) — the same module's own resolve path then rewrites that into the real download URL (`drive.usercontent.google.com/download?id=<id>&export=download&confirm=t`), but does not also recognize that already-rewritten URL as a valid INPUT. Confirmed live 2026-09-24 (WO-1040): `detect_platform()` returns `"unknown"` for a bare `drive.usercontent.google.com/download?id=...` URL (no extension in the URL either, so the generic `media_type()` extension fallback doesn't catch it), so `get_finder()` raises `UnsupportedPlatformError`.
  - **Impact**: `followup/approved_direct.csv` (WO-1040, 2026-09-24 hand-check batch) has several rows recorded with exactly this already-rewritten shape (whoever hand-verified them followed the link to its real download URL rather than keeping the original share link) — those rows cannot resolve/ingest/transcribe today even though the underlying file is real and was hand-confirmed. See WO-1040's PR for the exact affected domains/gov_ids.
  - **Next action**: extend `is_direct_file_url()` (and whatever internal rewrite exists at `direct_file.py`'s line ~390) to also recognize a `usercontent.google.com/download` URL directly — either resolve it as-is, or normalize it back to the canonical `id=` form before the existing rewrite logic runs.
  - **Constraint**: don't assume every `usercontent.google.com` URL is a Drive file without checking the `id=` query param is present — same discipline the existing `_DRIVE_FILE_ID_RE` regex already applies to the share-link shape.
  - **History**: found live building WO-1040; queue lines for the affected `approved_direct.csv` rows were queued anyway (Ryan's "never silently drop" rule) with this gap noted inline — they'll resolve automatically once this is fixed.
