...
- **[NEEDS-AUDIT] A "known platform, no page" sweep needs to filter out a government already represented in `scripts/tier3_auto_transcription_queue.txt` / `tier3_long_meetings_deferred.txt`, not just one with an existing Archive page — checking pages alone let WO-289 pick 5 of 7 hand-approved candidates that turned out to duplicate another concurrent sweep's already-queued meeting for the same government.**
  ...
  - **History**: `BACKLOG_DONE.md`'s WO-289 entry.
- **[NEEDS-AUDIT] `resolve_government()` matches a name that merely *contains* a real place/county name to that place/county, even when the raw name is a different government — confirmed live on 12 real Utah PMN entities in one run.**
  - **Issue**: found live 2026-09-12 (WO-299) resolving 59 Utah PMN "Entity" names that a first pass classified as already covered. Two shapes: a state-level body whose name merely contains the word "Utah" (e.g. `resolve_government("Utah Board of Higher Education, UT")`, `"Ascent Academies of Utah, UT"`, `"Utah State Fair Corporation Board of Directors, UT"`) resolves to **Utah County** (`us:county:49049`) — a coincidence of the state name and a county sharing the word "Utah" — and a special-service-area name that contains its host county/city's name (e.g. `"Ogden Valley Parks Service Area, UT"`, `"Salt Lake Valley Law Enforcement Service Area, UT"`, `"Summit County Service Area 3, UT"`) resolves to that county/city itself, even though a service area is its own separate government.
  - **Impact**: any caller that resolves a raw organization name (not a plain "City, ST" string) and trusts the result without checking that the resolved place/county name is a close match to the input risks silently attributing one government's data to a completely different one. WO-299 worked around this by hand (comparing the input text to the resolved name, normalized) rather than acting on the raw resolve — see `BACKLOG_DONE.md`'s WO-299 entry for the 12 excluded rows and the exact comparison used.
