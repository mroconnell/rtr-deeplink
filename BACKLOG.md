...
### State legislatures: 91 of 99 chamber rows still have no page — build one Sliq Harmony adapter first (7 states), then a TVW adapter (the Invintus hub walker for Oregon and Wisconsin is done, WO-922) `[JUST-DO-IT]` `[BIG]`
...
- **Next action:** (1) Sliq Harmony adapter: the Harmony pages are JavaScript-driven (`/Harmony/en/PowerBrowser/PowerBrowserV3/{date}/-1/{id}`), so first read one real event page from a live tenant (Kansas 00287 is the easiest) and confirm where the media URL and captions come from before writing a parser. (2) Invintus hub walker for Oregon and Wisconsin: DONE in WO-922 (`invintus.list_recent_events()` plus `passive_verify._invintus_walker`; after the next resolver deploy, queue the four hand-checked video-only candidates in BACKLOG_DONE.md WO-922, and run the walker on the other Invintus tenants). (3) TVW adapter for Washington. (4) Then the 49 "nothing found" rows by hand, one vendor at a time; use `research/wo919_report.csv` and `wo919_hub_vendors.csv` (rtr-business) as the worklist.
