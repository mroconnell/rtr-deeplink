...
- **Issue:** WO-919 (2026-09-20) ran the passive check and access ladder on all 99 chamber rows of the 2026-09-17 recon (`rtr-business/research/50_state_legislative_media_recon.md`; 49 two-chamber states plus unicameral Nebraska). 7 pages went live and 1 chamber already had pages, so 91 rows had no page that morning. Since then: the Sliq Harmony adapter was built (WO-921), all 13 of its tenants (see `app/platforms/sliq_harmony.py`'s `TENANT_JURISDICTIONS`) had exactly one meeting each until WO-1010; and the Invintus hub walker exists (WO-922). WO-1006 (2026-09-22) found Oklahoma's Senate tenant (00282) and five brand-new state legislature tenants (Maine, Iowa, Nevada, Missouri, Virginia). WO-1005 (2026-09-22, PR #1341) closed South Carolina/Tennessee/Nevada's Granicus blocks. **WO-1010 (2026-09-22) did two things**: (1) swept real depth into all 13 Sliq tenants — read a fresh batch of recent events per tenant, hand-verified each is a real committee/floor session/legislatively-created commission of that legislature (not a state board/agency sharing the venue — Oklahoma Senate's Workforce Commission/911 Management Authority/Medical Marijuana Authority and Nevada's PUCN/State Public Works Board/SPCSA were excluded on that basis), and ingested/queued 51 additional real meetings: 36 ingested with real captions, 7 queued for tier-3, 5 deferred (>90 min, swappable later), 3 skipped (New Mexico/Iowa streams flagged not enabled) — see `rtr-business/research/wo1010_handread.csv`. (2) Closed the Washington/TVW gap: WO-1009's recon found `tvw.org` is actually an Invintus tenant (clientID 9375922947) reached through a WordPress wrapper page, not a separate platform — `app/platforms/tvw.py` extracts the wrapper's `<meta name="clientID">`/`<meta name="eventID">` tags and delegates to `InvintusAssetFinder`; a real Senate Housing committee meeting with 1,447 real caption segments is now live. A THIRD real Invintus government pair also turned up on a different clientID (2917038973, CVTV — Clark/Vancouver Television) while investigating TVW: Vancouver, WA City Council and Clark County, WA Council, neither of which had any real video source before — both queued for tier-3. Checking nearby Invintus clientIDs (the same "try adjacent numbers" approach that found five new Sliq tenants) came back empty on all three known clientIDs' ±3 neighbors — confirmed live, Invintus's 10-digit clientIDs are not sequentially allocated the way Sliq's 5-digit tenant numbers are, so that particular shortcut doesn't transfer. The 91 is still stale and was not recounted: the Sliq/Invintus pages are filed under the state with a committee as meeting body, and the ingest CSVs record no chamber for them, so how many of the 99 rows they satisfy needs each committee read by hand.
...
- **Next action:** (1) Sliq: the 13 tenants now each have several real meetings (4-6 typically, fewer where the tenant genuinely has little else on-mission — Iowa/Missouri/Pennsylvania are thin on real supply, not under-swept); a later pass could go deeper still, but this is no longer "one meeting per state." Kansas/Iowa/Maine publish no captions on any meeting read across two sweeps now — treat that as a real per-tenant fact, not a fluke. (2) Invintus: TVW/Vancouver/Clark County need transcription (all queued); then re-run `list_recent_events()`/the walker on TVW (`clientID` 9375922947) and the CVTV tenant (2917038973) for depth the same way Sliq got it — both are large tenants (1,982 and 520+ recent events respectively) with plenty of real committee/council supply. Also worth a real government-hub vendor scan (WO-919's method: `player.invintus.com`/`eventlisting.invintus.com` embeds) rather than relying only on clientID-adjacency, now that adjacency is confirmed not to work. (3) Then the 49 "nothing found" rows by hand, one vendor at a time; use `research/wo919_report.csv` and `wo919_hub_vendors.csv` (rtr-business) as the worklist. (4) IL/MI/MS (TLS cert failures), CT/NH (timeouts), NY Senate (verification challenge) — the six chambers still genuinely blocked, no fix attempted yet.
