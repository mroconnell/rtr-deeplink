...
are two legitimate display forms of one government. 179 merges over 365
...
(`archive/data/hub_slug_aliases.csv`, 702 of them, generated by
...
| Legistar | `legistar.py` | Doesn't host video — finds the embedded/redirected link to a platform above (usually Granicus) and delegates via `resolve_via_platform()`. If its own `a.videolink` pattern finds nothing, falls back to a broader link scan (`base.find_platform_link()`, shared with the generic fallback below) before giving up — confirmed live on Baltimore's instance, whose real recording is a plain attachments-table link to YouTube, not the usual pattern. If *that* finds nothing either, a last-resort **city-YouTube-channel match** (`youtube_channel.py`, WO-30) covers the four confirmed instances where the page genuinely has no video to parse — Phoenix, Philadelphia, Baltimore and Albuquerque all publish recordings only to their own YouTube channel, unlinked from Legistar. It enumerates that city's known channel (a curated, human-verified netloc→channel-id dict, never a YouTube search) and matches the meeting's own body name and exact date against the video titles, declining on any ambiguity; a match carries a `video_warning` naming the channel and video it came from, since the meeting page never linked it. Last resort: a **same-tenant Granicus-listing match** (`granicus_channel.py`, added 2026-08-29 for Kansas City, MO, second tenant Yonkers, NY added 2026-09-10 WO-145) covers a different real shape — the recording genuinely exists on Granicus, the same platform the tenant's video already comes from when it's linked at all, just under a meeting id this page doesn't reference (Kansas City) or with no video mechanism wired up for a past meeting at all (Yonkers, whose `a.videolink` only gets a real link client-side for a *live* event). Matches the tenant's own public `ViewPublisherRSS.php` listing by body name and exact date, same decline-on-ambiguity posture as the YouTube-channel case. Curated per-tenant — see that module's docstring for the evidence and WO-145's read-only sweep of ~30 other Legistar tenants testing whether this generalizes | Whatever the delegated platform provides — for a channel match, YouTube's own captions |
| CivicPlus | `civicplus.py` | Same delegation pattern as Legistar, from AgendaCenter listing rows. Detected either by a `*.civicplus.com` netloc or, since WO-275 (2026-09-12), by the `/AgendaCenter` path alone — most real tenants are white-labeled onto the government's own domain and never touch civicplus.com, the same reason Hyland's own path-only check below exists | Whatever the delegated platform provides |
| Destiny AgendaQuick | `destinyhosted.py` | Confirmed live 2026-08-21 across 61 real `destinyhosted.com` tenants (enumerated via Wayback CDX — see BACKLOG_DONE.md) to be a pure agenda/minutes CMS, not a video host — delegates to `GenericFallbackAssetFinder`'s own tiers unchanged (a thin wrapper, not a redundant parser), only claiming the `"destinyhosted"` platform identity itself when nothing deeper resolves. Registered as its own platform (rather than left as `"unknown"`) specifically so `base.find_platform_link()` follows a `destinyhosted.com` link found on some *other* wrapper's page as a real one-more-hop delegation target — confirmed necessary live: Roswell, NM runs CivicPlus's AgendaCenter self-hosted on its own domain rather than `*.civicplus.com` and links straight to a `destinyhosted.com` backend. (Until WO-275, 2026-09-12, `detect_platform()`'s civicplus check only fired on a `*.civicplus.com` netloc, so a self-hosted AgendaCenter URL reaching it directly — not via `civicplus.py`'s own delegation — returned `"unknown"` rather than `"civicplus"`; it now also matches on the `/AgendaCenter` path shape, the same way the Hyland row below matches on `/Meetings/ViewMeeting` alone. This delegation from inside a civicplus.py resolve is unaffected either way.) 18/61 sampled tenants have a real, confirmed Swagit video link — via a formal built-in AgendaQuick↔Swagit integration, `onclick="swagitPlay('https://...')"` rather than a plain href — some others use YouTube/Granicus/Cablecast-Castus/other vendors, several have none this month (a real per-tenant negative, not a gap) | Whatever the delegated platform provides |
...
| CivicWeb (iCompass/Diligent) | `civicweb.py` | Confirmed live 2026-08-12 to be a YouTube-delegating platform, not a video host of its own — its "Video" tab embeds a plain YouTube iframe. Delegates straight to `YouTubeAssetFinder.resolve_video_id()` (the PrimeGov pattern: original CivicWeb URL preserved as `source_url`, `platform` stays `"youtube"`). **A second, genuinely different real URL shape** (found 2026-08-30 via a Wayback Machine CDX search, 108 real tenants confirmed): a direct `/document/{id}/` splitscreen-widget link, whose real meeting id lives only in the page's own inline JS config, not the URL — resolved via a second, richer API (`/api/geteventwithindexpoints/`) rather than the `Id=`-shape's `/api/videolink/` | Whatever YouTube provides. One real gotcha: `/api/videolink/{id}` is **double-JSON-encoded** — a first `.json()` parse yields a Python `str`, not the list it looks like; `_fetch_json()` re-parses if that happens. The `/document/{id}/` shape also gets real per-agenda-item deep-link timestamps (`agenda_items`) — the richer API's `LocalIndexPoints` field, matched against real `<a name="AgendaHeadingN">`/`<a name="AgendaItemN">` anchors already embedded in the document's own body HTML (see `civicweb.py`'s own API reference for the full field mapping) |
| Cablecast | `cablecast.py` | Confirmed live 2026-08-12 on Detroit's Cablecast portal, then confirmed 2026-08-13 that Charlotte, NC's real `/internetchannel/show/{id}` pages use the exact same Remix.js SSR template (all data, including a ~35-item "related shows" carousel, embedded in one `window.__remixContext` JSON blob) — one adapter, two confirmed real customers at build time. **Grown well past that since**: the `/coverage` page's own live jurisdiction data showed 17 real Cablecast tenants as of 2026-08-26 (`charlotte`/`concordca`/`coralvision`/`glendoraca`/`hctv`/`leonvalleytx`/`cityofpontiac`/`rialtoca`/`riverviewmi`/`salem`/`cityofsantee`/`city-slp-mn`/`virginiabeach`/`wilmington-nc`/`winchendon`.cablecast.tv among them, spot-checked live the same day), split across both the old `/internetchannel/show/{id}` path and the newer bare `/show/{id}` path this adapter already handles (`_SHOW_ID_RE`/`_SHOW_ID_SHORT_RE`) — still **deliberately not a general `*.cablecast.tv` rule**, since an unconfirmed customer could still use a different template, but the two-template split has held across a much bigger real sample than the original two. Jurisdiction is derived per-customer (see `_extract_jurisdiction()`), not hardcoded. Real quirk: Detroit's portal HTTPS hangs indefinitely for the whole domain (15s+ confirmed via direct curl) — `resolve()` always fetches over plain HTTP regardless of the scheme pasted, matching how Detroit's own city site (detroitmi.gov) actually links it. The real video is a direct, unauthenticated `.m3u8` on a separate `reflect-detroit-vod.cablecast.tv` subdomain (HTTPS works fine there), already fully supported by the existing hls.js pathway | `vodTranscripts` was empty on every one of 36 Detroit shows checked, but a real, populated, fetchable entry was found on a Charlotte show (`show/2451`) — a plain-text transcript file, parsed directly (`_parse_transcript()`), not through the shared VTT dispatch |
| CHAMP/ChampDS | `champds.py` | Confirmed live 2026-08-13 against 6 independent real customers via a plain, unauthenticated JSON API (`playapi.champds.com/{customer}/event/{id}`). Only the direct-MP4 `MediaInfo.DownloadURL` case is wired up to play (2 of 6 customers). The majority-case `VOD2` HLS stream answers only requests with `Referer: https://play.champds.com/`, which a reader's browser can't send, so it never goes in `video_url`. Since WO-1045 it is returned as `server_media_url` instead: the transcription paths read it (our server can send that Referer), and the reader sees a "stream-only" warning rather than "No video found". Since WO-1052 the stream is set even when a `DownloadURL` exists, and our own server-side reads (transcription, the queue probe) prefer it: many of these MP4s keep their index at the end of the file, which ChampDS's server takes minutes to reach | Real VTT captions from `MediaInfo.Captions`, fetched from `play.champds.com/CAPTION/{customer}{MediaPath}` (WO-1045; confirmed on El Paso County CO and Atlanta GA). Plus `Agenda.Attachments` as `agenda_link`, preferring a `.pdf`-shaped one |
...
| 12milesout.com (`{tenant}.12milesout.com`) | `twelvemilesout.py` | Confirmed live 2026-09-24 (WO-1047) across 6 real San Diego/Inland Empire, CA tenants (Escondido, Coronado, Colton, Covina, Big Bear Lake, Solana Beach) — one meeting per `{tenant}.12milesout.com/.../{id}` page, tenancy per-subdomain. A direct, unauthenticated CloudFront MP4 download link on every meeting page (`<a href="...mp4" download>`), not the JW Player/HLS `smil:` URL also present (its CDN host has no live DNS). Real `#agenda-list` chapter markers (`data-start`/`data-duration`) present on every tenant regardless of captions. Jurisdiction from a small curated tenant map (subdomains don't self-describe) | A hidden `_su` field (base64, pipe-delimited: server URL/file id/aspect ratio/duration/video id/thumbnail/permalink) plus the `<video>` tag's own `data-captions="{codes}"` attribute (comma-separated ISO 639-1 codes, e.g. "en,es") drive a plain, unauthenticated GET to `/api/captions/{fileId}/{code}/0/{durationMs}` — confirmed live returning real WebVTT (1,874 cues, Coronado). Empty/absent `data-captions` (confirmed on Covina/Escondido) means no captions, not an error |
| spectrumstream.com (`spectrumstream.com/streaming/{tenant}/`) | `spectrumstream.py` | Confirmed live 2026-09-24 (WO-1047) across 5 real LA-area CA tenants (Alhambra, Arcadia, South Pasadena [+committees], Glendale USD, Burbank-Glendale-Pasadena Airport Authority) — tenancy is PATH-based on one DNS-wildcarded domain, unlike 12milesout.com above. Every tenant shares one legacy ColdFusion template: a `jwplayer("myElement").setup({file: "https://spectrum_streaming.s3.amazonaws.com/{tenant}/..."})` inline script (a direct S3 mp4) and a `<a onclick="jwplayer().seek({s})">` agenda list. A tenant's bare root is a truncated fragment carrying only the newest meeting's filename (no real listing) — the listing walker bootstraps from it, or from a known-real seed page when even that's absent (South Pasadena's root was replaced by a live-only Castr embed, its own archive pages unaffected) | When present, the same jwplayer config's `tracks[].file` is a real `.vtt` URL (confirmed live, Alhambra: real English captions). Two real "no captions" shapes, both non-errors: `tracks` present with an empty `file` string (Glendale USD), and no `tracks` key at all (South Pasadena, BGPAA) |
...
| Tampa, FL City Council transcripts | `tampa.py` | Confirmed live 2026-08-30 (WO-73) across 4 real meetings spanning 2022–2026. A single-city platform (`apps.tampagov.net`'s own "CTTV" webapp), not a vendor product. Each transcript detail page (`Agenda.aspx?pkey={N}`) embeds its own paired YouTube video directly in a sidebar card — no cross-referencing against the 2,611-row paginated listing grid needed (that grid's own pagination is real-postback-only ASP.NET WebForms, confirmed walkable with plain `aiohttp` POSTs if ever needed, but wasn't). Delegates to `YouTubeAssetFinder` for playback (the PrimeGov/CivicWeb pattern: original `apps.tampagov.net` URL kept as `source_url`, `platform` reset back to `"tampa"` afterward, the ChicagoElms variant). A bare listing-page URL (no `?pkey=`) returns the 50 most recent meetings as `CalendarPageError` candidates, same shape as Legistar's `Calendar.aspx` | Real, per-utterance HH:MM:SS-timestamped closed captioning parsed directly from the transcript page itself — genuinely higher quality than YouTube's own auto-captions for the same video (confirmed live: YouTube's auto-caption track for one sample was almost entirely `[Music]`/filler noise, Tampa's own transcript had clean real dialogue with speaker names), so `segments` always prefers this page's own transcript over whatever YouTube's delegated resolve found. Two real timestamp-id shapes handled (24-hour `t090416`, 12-hour `t50414PM`) |
| CivicLive (`{tenant}.hosted[2].civiclive.com`) | `civiclive.py` | Confirmed live 2026-09-01 (WO-92) — a real, 1000+-customer municipal CMS (Intrafinity, formerly West Corp) with no video product of its own, same delegation shape as CivicPlus/Destiny AgendaQuick. Both `hosted`/`hosted2` subdomains are genuinely Cloudflare-wildcarded at DNS/TLS (unlike civicplus.com/primegov.com), so tenant enumeration goes through a Wayback CDX domain-wide scan rather than subdomain guessing — see `civiclive.py`'s own module docstring. Real per-meeting agenda/calendar content turned out to be client-rendered on every tenant checked (Auburn WA, Escalon CA) and invisible to a plain fetch, so this doesn't attempt CivicPlus-style row scraping; instead it (1) fetches with redirects and re-checks the *final* URL's host — Auburn's own "Agendas & Minutes" page is a genuine plain-HTTP 302 straight to `auburnwa.portal.civicclerk.com`, delegated via `resolve_via_platform()` — and (2) otherwise delegates wholesale to `GenericFallbackAssetFinder` (the `destinyhosted.py` thin-wrapper pattern), which already excludes a bare YouTube channel/handle link (confirmed real and common — Auburn/Escalon both only publish a channel-level link, never a per-meeting one) from being mistaken for a specific video | Whatever the delegated platform provides |
| Utah Public Notice Website (`utah.gov/pmn`) | `utah_pmn.py` | Confirmed live 2026-09-08/09 via a full statewide enumeration pilot (`scripts/pmn_utah_pilot.py`, `ENUMERATION_METHODS.md` §105) — Utah's statutory Open and Public Meetings Act notice board, where every Utah government type (state agencies, counties, municipalities, special districts, school districts, and more) is legally required to post meeting notices with self-reported Government Type/Entity/Public Body fields. Two real media shapes: a notice's "Audio File Location" field most often links OFF utah.gov (YouTube/Vimeo/CivicClerk/etc) — delegates via `resolve_via_platform()`, keeping the PMN notice page as `source_url` and overriding jurisdiction/meeting_body/title/date with PMN's own ground-truth fields rather than trusting the delegate's own guess; or the notice's own "Download Attachments" table has a bare `.m4a`/`.mp3`/`.mp4` file hosted directly on utah.gov with no platform wrapper at all — a shape no other adapter here has needed before, resolved by setting `video_url` directly to the file with `video_format` inferred from its extension, no new transcription-pipeline work needed | Never resolved directly by this adapter — either whatever the delegated platform provides, or (the bare-file case) real segments only after the on-demand Whisper pipeline transcribes it, same as any other audio-only source (see `media_probe.py`'s `probe_has_video_stream()`) |
| Wistia (`{account}.wistia.com`) | `wistia.py` | Confirmed live 2026-09-10 (WO-161) against RegionalWebTV/Advanced Media Solutions of Virginia's shared `amsva.wistia.com` account — real, independent Virginia governments Warrenton (town), Manassas (city), Fredericksburg (city), and Stafford County. A general-purpose video host (Vimeo's shape), not a civic-video vendor — `detect_platform()` only claims a real `/medias/{id}` or `/channel/{id}` URL on a per-account subdomain, never the bare `wistia.com` marketing site. A direct media/channel URL resolves straight from Wistia's own public, unauthenticated JSON API (`fast.wistia.com/embed/medias\|channel/{id}.json`); a **delegating government page** (any URL, not just RegionalWebTV's) is scanned for four real embed shapes (`fast.wistia.com/embed/medias/{id}.jsonp`, `fast.wistia.net/embed/iframe/{id}`, `class="wistia_embed\|wistia_channel wistia_async_{id}"`, a plain `<a href="{account}.wistia.com/medias/{id}">`), keeping the government page as `source_url`. RegionalWebTV's own current-year pages embed the whole channel via a Wix custom-HTML widget invisible to a plain fetch — a headless-browser render (same Playwright path Vimeo's captions use) finds the real nested custom-HTML document, then a plain fetch reads it, no further JS needed. A dead/pruned id answers HTTP 200 with `{"error": true}` (confirmed on every 2019/2021 id checked from RegionalWebTV's own archive pages) — raised as a real not-found, never a silent empty page; the sweep target is this year's meetings, not the historical archive. A channel URL/embed returns a real `calendar_page` pick-list, same shape as Vimeo's showcase/channel listings | **Real, unauthenticated WebVTT with real timestamps** (`fast.wistia.com/embed/captions/{id}.vtt?language=eng`) — unlike Vimeo, no headless browser or signed URL needed for captions at all. `video_url` is a real, public, byte-range-capable MP4 delivery URL straight from the media JSON (also unlike Vimeo, whose real media file is walled off) |
| BoxCast (`boxcast.tv`) | `boxcast.py` | Confirmed live 2026-09-11 (WO-227) against four independent governments — Wilmington OH, St. Louis County (Clayton MO), Hondo TX, Atlantic City NJ. A general-purpose government video vendor, reached via a real, public, unauthenticated REST API (`rest.boxcast.com`), not page scraping. Three real URL shapes: `boxcast.tv/channel/{id}` (a government's whole channel — auto-picks the newest past broadcast whose title looks like a real meeting, via a shared phrase-list hand-check plus a governing-body keyword gate, since a channel's own recent broadcasts often include ceremonies/press-conferences/ribbon-cuttings mixed in with real meetings) and `boxcast.tv/view/{slug}`/`boxcast.tv/view-embed/{slug}` (one specific broadcast, resolved directly with no filtering). `video_url` is a real, signed, expiring HLS master playlist fetched fresh at resolve time (re-resolved fresh by the worker before transcribing, same as every other signed-URL platform here). **The Archive never trusts that stored URL past ingest, either** (WO-229, 2026-09-11): a BoxCast page's own stored `video_url` expires a couple of days after ingest, so the player is pointed at `/m/{slug}/video`, which resolves a fresh signed playlist at view time (cached briefly) and 302s there, falling back to the stored URL if BoxCast can't be reached — see `archive/utils/video_refresh.py`. Jurisdiction and the stable per-government `external_id` (used for `tenant_overrides.csv` pins, since `boxcast.tv` is a shared multi-government host) both come from the broadcast's own BoxCast *account*, never guessed from a title | **Real, server-fetchable WebVTT captions — on some tenants.** A real, paid, per-account BoxCast add-on (confirmed: Atlantic City and St. Louis County have it, Wilmington and Hondo don't) delivered as a genuine `#EXT-X-MEDIA:TYPE=SUBTITLES` HLS track, itself split into hundreds of ~12-second signed WebVTT segments (533 for one real 107-minute meeting) fetched with bounded concurrency and concatenated — each segment's own cue timestamps are already correct absolute positions in the recording, confirmed live rather than assumed. No caption track at all is a normal, honest outcome on the other tenants |
| BoardDocs (`go.boarddocs.com`) | `boarddocs.py` | Built WO-365, 2026-09-14, from the mechanism spec'd in `docs/investigations/boarddocs_video_adapter.md`. Mostly a school-board agenda platform (969 of 972 research rows mentioning it are school districts) with no video at all — but where a tenant's own `bd.videoservice` flag is on (YouTube/Vimeo), a fixed, unauthenticated per-meeting mechanism exposes the real video id: a tenant Public page for the flag and org name, `BD-GETMeetingsListForSEO` for the meeting list (newest first, includes future meetings), then one `POST .../VIDEO-GetAgenda` per candidate meeting (its own real `Unique` id) until one comes back with a video, capped at 40 — same delegation pattern as PrimeGov (`YouTubeAssetFinder`/`VimeoAssetFinder.resolve_video_id()` called directly, original BoardDocs meeting URL kept as `source_url`, `platform` stays `"youtube"`/`"vimeo"`). **On-demand only, never a sweep**: `go.boarddocs.com/robots.txt` disallows every agent with `Crawl-delay: 1000`, so this adapter reads exactly the one tenant/meeting a resolve asks for — no script in this repo may loop it over tenants. Confirmed live on two real tenants (Tallahassee FL's City Commission, 6 of 6 recent meetings have video; Colorado City AZ schools, 5 of 40) and four negative controls (Austin ISD, Northside ISD, St. Charles R-VI MO, Vacaville USD — flag on, zero real video ids on the 40 newest, degrades cleanly to "no video" rather than erroring) | Whatever the delegated platform provides — no confirmed BoardDocs-native caption track exists (unclaimed, not silently assumed). The `VIDEO-GetAgenda` fragment's own agenda outline is real and richer than the spec first suggested: each timed `<li class="item" data-videohours/minutes/seconds>` is a genuine per-item offset into the video, turned into `agenda_items` (a video-less meeting still returns its category headings, just with no timed item rows at all) |
| Sliq Harmony (`sg001-harmony.sliq.net/{tenant}/Harmony/...`) | `sliq_harmony.py` | Built WO-921, 2026-09-20, against real pages from all seven state-legislature tenants that use it: Arkansas 00284, Colorado 00327, Delaware 00329, Kansas 00287, New Mexico 00293, Oklahoma House 00283, West Virginia Senate 00289 (82 recent events resolved live through the adapter with zero errors; 10 saved real event pages and 1 listing are the test fixtures). One shared host, the government is a five-digit tenant path segment, so identity is pinned per tenant path (`tenant_overrides.csv`, `us:state:NN`, `source=wo921`), never by host; the chamber or committee is `meeting_body` (D1). The event page is not JavaScript-only: the server writes a `dataModel` block into the HTML, so ONE plain GET returns the HLS `playlist.m3u8` (open CORS, no cookie, plays in hls.js), the closed captions, a timed agenda and the meeting times. A listing URL (`.../View/`, `.../View/RecentEnded/`) raises a pick-list (`CalendarPageError`). No video (not yet held, or a stream flagged `Enabled: false` with a `novod.com` or empty placeholder) resolves as a real meeting with a plain video warning | Real, timed captions where the legislature publishes them: `ccItems` carries absolute wall-clock `Begin`/`End`, turned into offsets from `mediaStartTime`. Coverage is uneven and reported as it is: Colorado, Oklahoma, West Virginia, Arkansas, Delaware and New Mexico publish captions on most meetings; **Kansas published none on any of the 12 recent meetings read** (video-only, so its pages go to the tier-3 transcription queue); every tenant has some captionless meetings. The agenda (`AgendaTree`, nested, timed) becomes `agenda_items`; WV/AR `Speakers` turns are left out on purpose |
| Invintus (`player.invintus.com`) | `invintus.py` | Public `Event/getDetailed` API: the direct MP4 download link, else the HLS stream (`streamingURIs.main`) | Two ways. (1) A WebVTT caption file in `captionPath`. (2) Since WO-1065 (2026-09-25): CEA-608 captions carried inside the HLS video itself, with no caption file. When `captionPath` is empty, `resolve()` reads up to 3 six-second pieces spread through the smallest (160p) stream and runs ffmpeg's caption reader on each (`embedded_captions.py`). If real words come out, the page says captions are embedded and not yet text. The cloud worker then reads the whole stream (about 210 MB per hour of meeting) and stores the captions as the page's transcript. Caption markers alone prove nothing: they sit on every frame, even in silence. |
...
**BoardDocs** (mostly a school-board agenda platform, but where a
tenant turns its video service on, a fixed unauthenticated mechanism
exposes a per-meeting YouTube/Vimeo id) is built — see the "Supported
platforms" table above and `docs/investigations/boarddocs_video_adapter.md`
for the full mechanism and house rule.
