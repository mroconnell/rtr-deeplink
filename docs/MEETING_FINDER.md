...
(`app/platforms/meeting_finder/identify.py`) as a standalone module, and
WO-1028 (2026-09-23) built **List** (`app/platforms/meeting_finder/
listing.py`'s `list_account()`) -- see this doc's own List section below
for the five listers and the order they're tried in. Neither is yet
wired into `runner.py` -- a later WO plugs both (and Scan/Hop/Start,
built in the same wave by other agents) into the pipeline. **`resolve`
is still the only entry `runner.py` itself drives end to end** --
`start`/`identify`/`list`/`scan` are accepted by the CLI/`FinderInput`
but return the outcome `phase-not-built` until that wiring WO lands. See
