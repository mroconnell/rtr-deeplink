...
- **`CLAUDE_INBOX_TRIAGE.md` is a third, separate staging file — set up
  ...
  by an interactive session.** The Routine reads Gmail's `rtr-Codex`
  ...
  **The Routine dedupes against a repo-side ledger, not a Gmail label
  ...
  the old `label:rtr-Codex -label:rtr-Codex-processed` query are gone
...
`CLAUDE_BACKLOG.md` for unreviewed, Codex-proposed ideas) — deliberately,
...
`~/Documents/rtr-business` (its own AGENTS.md, task backlog, and topic
folders, plus a linked Codex.ai Project). Product *roadmap* items still
...
day-by-day investigation detail) lives in Codex auto-memory scoped
to `~/Documents/rtr-transcript` — the *original* project directory this
one was spun out of, not this one. A Codex session opened at
